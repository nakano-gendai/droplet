...
module globals
    ...
    integer,parameter:: xmax = 65 !ｘ方向格子数（０から数える）
    integer,parameter:: ymax = 65 !ｙ方向格子数（０から数える）
    integer,parameter:: zmax = 65 !ｚ方向格子数（０から数える）
    ...
    integer,parameter:: step = 15000 !計算時間step
    integer,parameter:: start = 1000000000 !壁を動かし始める時間step
    integer,parameter:: kappag_start = 20 !界面張力をかけ始めるstep
    integer,parameter:: kappag_end = 10000 !界面張力をかけ終わるstep
    integer i, j, k, n, nall, nstar, xi, yi, zi, alpha, beta
    character :: filename*200
    character :: filename2*200
    character :: filename3*200
    ! character(*),parameter :: datadir = "/data/sht/nakanog/newtest3/ini_re1000we2/"
    ! character(*),parameter :: datadir2 = "/data/sht/nakanog/newtest3/ini_re1000we2/fg/"
    character(*),parameter :: datadir = "/data/2022/nakano/sigmatest6/"
    character(*),parameter :: datadir2 = "/data/2022/nakano/sigmatest6/fg/"
    character(*),parameter :: datadir3 = "/data/n/n517/newtest2/ini_re1000we2/fg/"
...
DO n=1,step
    if(n < kappag_start) then
        ...
        kappagtmp = dble(n-kappag_start)/dble(kappag_end-kappag_start)*kappag
    else
        kappagtmp = kappag
    end if
    ! if(n == 5000) then
    !     write(filename,*) group_new !i->filename 変換
    !     write(filename3,*) new_rank
    !     filename=datadir3//'0_'//trim(adjustl(filename3))//'_300000_fg.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
    !     print *, filename !表示してみる
...
!==========================物理量の計算==========================================================
    ...
    if(comm_rank == 0) then
        ...
        if(mod(n,10)==0) then
            if(n == 10) then
                open(10,file="./time.d")
                write(10,*) n, time2-time1, u1_procs(1,1,1), kappagtmp
                close(10)
            else
    ...
    !     endif
        ...
        !     do alpha=1, 3
        !     do beta=1, 3
        !     write(11,*) xi,yi,zi,gphi_procs(alpha,beta,xi,yi,zi)
        !     enddo
        !     enddo
        !     enddo
        !     ! enddo
