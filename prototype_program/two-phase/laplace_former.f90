! このサンプルプログラムは，「静止液滴」のプログラムで，
! 以下の書籍に付随するものです．
!   格子ボルツマン法入門：複雑境界および移動境界流れの数値計算法
!   稲室，吉野，鈴木
!   ISBN 978-4-621-30476-1
!   https://www.maruzen-publishing.co.jp/item/?book_no=303613
!
!  Copyright @ 2020 Kosuke Suzuki. All rights reserved.
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
program laplace
 ...
 integer, parameter:: imax = 65 	!格子数（x方向）
 integer, parameter:: jmax = 65 	!格子数（y方向）
 integer, parameter:: kmax = 65 	!格子数（z方向）

 real(8), parameter:: D = 32.d0*ds	!初期液滴直径
 ...
 !====================================================================
 !=====================その他定数・変数===============================
 integer, parameter:: step = 1000000		!時間ステップ上限
 ...
 real(8):: err, distance, seff, L1, B1, Dd				!Laplace圧からの誤差
 ...
 do k=0,kmax
     do i=0,imax
         if(phi(i,j,k) >= 0.5d0*(phi1+phi2)) then
             seff = seff + ds**2
             if( distance <= sqrt((dble(i)-xc)**2 + (dble(k)-zc)**2) ) then
                 distance = sqrt((dble(i)-xc)**2 + (dble(k)-zc)**2)
             endif
         endif
     enddo
 enddo
...
B1 = (4.0d0*seff) / (pi*L1)
Dd = (L1-B1) / (L1+B1)
write(*,*) "step=",n,veff,seff,L1,B1,Dd

 write(8,'(1i10, 1x, 3f20.15)') n, dp_calc, dp_th, err
 !-------------------------------------------
 !---------実効液滴体積・直径----------------
!  write(9,'(1i10, 1x, 2f20.15)') n, veff, reff
 write(9,"(4es16.8)") dble(n), veff, reff, dp_calc
 !-------------------------------------------
