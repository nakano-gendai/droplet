module globals
    ...
    integer,parameter:: start = 100000000 !壁を動かす時間step
    integer,parameter:: kappag_start = 5000 !界面張力をかけ始めるstep
    integer,parameter:: kappag_end = 100000 !界面張力をかけ終わるstep
...
contains
    ...
    subroutine physics(phi_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,g_procs,nu_procs,taug_procs,grad_phi_procs)
        real(8),intent(inout):: phi_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        ...
        real(8),intent(inout):: taug_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(inout):: grad_phi_procs(1:3,1:x_procs,1:ymax+1,1:y_procs)
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    ...
                    enddo
                    p_procs(xi,yi,zi) = p_procs(xi,yi,zi) + 2.0d0/3.0d0*kappag_tmp*(grad_phi_procs(1,xi,yi,zi)**2+grad_phi_procs(2,xi,yi,zi)**2+grad_phi_procs(3,xi,yi,zi)**2)
...
!===========taylor-flow=============================================
        ...
        !     do yi=0,ymax
        !         do xi=0,xmax
        !             ut1 = -u0*exp(-nu1*dble(n)*(k1**2+k3**2))*cos(k1*dble(xi))*sin(k3*dble(zi))
        !             ut3 = k1/k3*u0*exp(-nu1*dble(n)*(k1**2+k3**2))*sin(k1*dble(xi))*cos(k3*dble(zi))
        !             pt = p0 -rho/4.0d0*exp(-2.0d0*nu1*dble(n)*(k1**2+k3**2))*(u0**2*cos(2.0d0*k1*dble(xi))+ &
...
!==================徐々に界面張力を印加する=================================================
    ...
    !         do yi=1,ymax+1
    !             do xi=1,x_procs
    !                 if(phi_procs(xi,yi,zi)>=4.0d0) then
    !                     p_procs(xi,yi,zi) = 2.185d-4
    !                 endif
    !             enddo
    !         enddo
    !     enddo
...
!==========================物理量の計算==========================================================
    call physics(phi_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,g_procs,nu_procs,taug_procs,grad_phi_procs)
!============================物理量をまとめる======================================================
    !p
    tmp(:,:,:) = 0.0d0
    do zi=1,y_procs
...
!============================================================================================
    ! if(n==step) then
    !     if(comm_rank==0)then
    !         open(50,file="couette.d")
    !         xi = 1
    !         yi = 1
    !         do zi=0,zmax
    !             write(50,"(5es16.8)") dble(zi),u1(xi,yi,zi),u2(xi,yi,zi),u3(xi,yi,zi),p(xi,yi,zi)
    !         enddo
    !         close(50)
    !         call output(phi,p,u1,u2,u3)
    !     endif
