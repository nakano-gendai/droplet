...
module globals
    ...
    !出力ディレクトリ
    character(*),parameter :: datadir = "/data/n/n517/re500we7_box_d-5/"
    character(*),parameter :: datadir2 = "/data/n/n517/re500we7_box_d-5/fg/"
    character(*),parameter :: datadir_ran = "/data/n/n517/re500we7_box_d-5/ran/"
    !入力ディレクトリ
    character(*),parameter :: datadir_ini = "/data/n/n517/ini_0.05_999_290_290/fg/"
...
!============================一様せん断流の入力（撹乱を添加）=========================================
        write(filename,*) group_new !i->filename 変換
        write(filename3,*) new_rank
        filename=datadir_ini//'0_'//trim(adjustl(filename3))//'_200000_fg.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
        print *, filename !表示してみる
...
!================================出力==================================
    ...
    if(comm_rank == 0) then
        ...
        if(mod(n,100)==0) then
            if(n == 1000) then
                open(10,file="./time1.d")
                write(10,*) n, time2-time1, u1_procs(1,1,1)
                close(10)
            else
                open(10,file="./time1.d",action="write",position="append")
                write(10,*) n, time2-time1, u1_procs(1,1,1)
                close(10)
            endif
        endif
    endif
    if(comm_rank == 2500) then
        call cpu_time(time2)
        if(mod(n,100)==0) then
            if(n == 1000) then
                ...
                open(10,file="./time2.d",action="write",position="append")
                write(10,*) n, time2-time1, u1_procs(1,1,1)
                close(10)
            endif
        endif
    endif
    if(comm_rank == 7000) then
        call cpu_time(time2)
        if(mod(n,100)==0) then
            if(n == 1000) then
                open(10,file="./time3.d")
                write(10,*) n, time2-time1, u1_procs(1,1,1)
                close(10)
            else
                open(10,file="./time3.d",action="write",position="append")
                write(10,*) n, time2-time1, u1_procs(1,1,1)
                close(10)
            endif
        endif
...
!一様乱数の発生のプログラム（ただし、seedsize = 1 の場合）
! call system_clock(seed)
! seed = seed + comm_rank
! seeds(1) = seed
! call random_seed(put=seeds)
