module globals
    ...
    integer,parameter:: xmax = 62 !ｘ方向格子数
    integer,parameter:: ymax = 62 !ｙ方向格子数
    integer,parameter:: zmax = 62 !ｚ方向格子数
    integer,parameter:: step = 500000 !計算時間step
    ...
    !支配パラメータ
    real(8),parameter:: D = 30.0d0 * ds !設置する液滴のサイズ
    real(8),parameter:: nu1 = 1.0d-1 !液体の動粘度
    real(8),parameter:: nu2 = 1.0d-1 !液滴の動粘度
    real(8),parameter:: uw = 5.0d-3 !壁の移動速度
    real(8),parameter:: kappaf = 0.01d0*ds**2 !界面厚さを決めるパラメータ
    real(8),parameter:: kappag = 1.0d-3 !界面張力を決まるパラメータ
...
contains
    ...
    subroutine bounce_back_LKS(fun,U)
        real(8),intent(inout):: fun(1:15,0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(inout):: U(1:3)
        !安定したら壁を動かす
        if((n >= start)) then
    ...
    end subroutine bounce_back_LKS
    ...
    subroutine output(phi,p,u1,u2,u3)
        ...
        do zi=0,zmax
            do xi=0,xmax
                write(20,"(8es16.8)") dble(xi),dble(zi),u1(xi,yi,zi),u2(xi,yi,zi),u3(xi,yi,zi),sqrt(u1(xi,yi,zi)**2+u2(xi,yi,zi)**2+u3(xi,yi,zi)**2),p(xi,yi,zi)
            enddo
            write(20,*)
        enddo
        close(20)
...
!=======================境界条件=============================================================
    ! call bounce_back_LKS(feq_procs,U)
