module globals
    ...
    integer,parameter:: xmax = 65 !ｘ方向格子数
    integer,parameter:: ymax = 65 !ｙ方向格子数
    integer,parameter:: zmax = 65 !ｚ方向格子数
    integer,parameter:: step = 50000 !計算時間step
    integer,parameter:: start = 20000000 !壁を動かし始める時間step
    integer,parameter:: kappag_start = 1000 !界面張力をかけ始めるstep
    integer,parameter:: kappag_end = 21000 !界面張力をかけ終わるstep
    integer i, n, xi, yi, zi, alpha, beta, k
    character :: filename*200
    ! character(*),parameter :: datadir = "./data_phi/"
    ! character(*),parameter :: datadir = "/home/nakano/aomsin/data_aomsin/"
    character(*),parameter :: datadir = "/data/2022/nakano/data_sekisann/"
    ...
    real(8) time1,time2
    real(8) kappag_tmp
    real(8) theta, xj, zj,  phi_hokann
    integer x_tmp, z_tmp, x1, x2, z1, z2
    real(8),parameter :: zz = 0.1d0
    real(8),parameter :: xx = 0.1d0
    real(8) xg, yg, zg, phi_tmp
...
contains
    ...
    subroutine MPI_boundary(phi_procs,f_procs,g_procs,taug_procs)
    ...
    subroutine laplace(phi,p,grad_phi)
        real(8),intent(inout):: phi(-1:xmax+1,-1:ymax+1,-1:zmax+1)
        real(8),intent(in):: p(0:xmax,0:ymax,0:zmax)
        real(8),intent(inout):: grad_phi(1:3,0:xmax,0:ymax,0:zmax)
        !圧力差を求めて表面張力をラプラスの式から計算 phi1 = 2.211 phi2 = 4.895
        veff = 0.0d0
        do zi=0,zmax
            do yi=0,ymax
                do xi=0,xmax
        ...
        p_in = p((xmax+1)/2,(ymax+1)/2,(zmax+1)/2)
        ...
        sigma_th = 0.0d0
        do xi=0,(xmax+1)/2
            sigma_th = sigma_th + grad_phi(1,xi,yi,zi)**2
        enddo
        sigma_th = sigma_th * kappag_tmp
        ! if(n==10000) then
        !     open(91,file="grad_phi.d")
        !     do xi=0,(xmax+1)/2
        !         write(91,"(6es16.8)") dble(xi),dble(yi),dble(zi),grad_phi(1,xi,yi,zi),grad_phi(2,xi,yi,zi),grad_phi(3,xi,yi,zi)
        !     enddo
        !     close(91)
        ! endif
        ...
        !     enddo
        ! enddo
        ! write(8,"(3es24.16)") dble(n),reff,wa
    ...
    subroutine break_up(phi)
        ...
        B1 = (4.0d0*seff) / (pi*L1)
        Dd = (L1-B1) / (L1+B1)
        write(9,"(7es16.8)") dble(n),(dble(n)-dble(start))*2.0d0*uw/H1,veff,seff,L1,B1,Dd
    end subroutine break_up

    subroutine break_up2(phi)
        real(8),intent(in):: phi(-1:xmax+1,-1:ymax+1,-1:zmax+1)
        !重心座標を計算
        xg = 0.0d0
        yg = 0.0d0
        ...
        do zi=0,zmax
            do yi=0,ymax
                do xi=0,xmax
                    if(phi(xi,yi,zi) >= 0.5d0*(phi1+phi2)) then
                        xg = xg + dble(xi)*phi(xi,yi,zi)
                        yg = yg + dble(yi)*phi(xi,yi,zi)
                        zg = zg + dble(zi)*phi(xi,yi,zi)
                        phi_tmp = phi_tmp + phi(xi,yi,zi)
                    endif
                enddo
            enddo
        enddo
        ...
        do zi=0,zmax
            ...
            enddo
        enddo
        theta = abs( acos((dble(x_tmp) - xg) / distance) )
        ...
        do zi=0,zmax
            ...
            enddo
        enddo
        write(8,"(4es24.16)") dble(n),L1,B1,wa
        ! write(9,"(6es16.8)") dble(n),(dble(n)-dble(start))*2.0d0*uw/H1,L1,B1,Dd,acos((dble(x_tmp) - xc) / distance)
        write(9,"(8es16.8)") dble(n),(dble(n)-dble(start))*2.0d0*uw/H1,L1,B1,Dd,dble(xg),dble(yg),dble(zg)
    end subroutine break_up2
...
!==================徐々に界面張力を印加する=================================================
    ...
    kappag_tmp = kappag
!======================のりしろ境界==========================================================
    call MPI_boundary(phi_procs,f_procs,g_procs,taug_procs)
!=======================div/gradの計算============================================================
