...
module globals
    ...
    integer,parameter:: step = 150000 !計算時間step
    ...
    ! character(*),parameter :: datadir = "/data/n/n517/crit/re150/"
    character(*),parameter :: datadir = "/data/n/n517/multitest/"
    ...
    real(8), allocatable:: tmp2(:,:,:,:),tmp3(:,:,:,:),tmp4(:,:,:,:),tmp5(:,:,:,:),tmpff(:,:,:,:,:)
...
contains
    ...
    !========================並列数・コミュニケータ分割・通信先設定========================
        ...
        Nx = 43 !x方向の並列数（ただし，Nx/=comm_procs）
    ...
    !===============================================================================================================
        ...
        if(new_rank == 0) then
            ...
            allocate(grad_u(1:3,1:3,0:xmax,0:ymax,0:zmax))
        endif
        write(*,*) "2"
        !以下はのりしろ有りの変数
        ...
        ! write(*,*) "6"
        ! call fromtmp(u1_procs)
        ! write(*,*) "7"
        ! if(new_rank == 0) then
        !     call totmp(u2)
        ! endif
        ! call MPI_Bcast(tmp1(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        ! call fromtmp(u2_procs)

        ! if(new_rank == 0) then
        !     call totmp(u3)
        ! endif
        ! call MPI_Bcast(tmp1(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        ! call fromtmp(u3_procs)

        ! if(new_rank == 0) then
        !     call totmp(p)
        ! endif
        ! call MPI_Bcast(tmp1(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        ! call fromtmp(p_procs)

        ! if(new_rank == 0) then
        !     call totmp(phi)
        ! endif
        ! call MPI_Bcast(tmp1(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        ! call fromtmp(phi_procs)

        ! if(new_rank == 0) then
        !     call totmpf(feq)
        ! endif
        ! call MPI_Bcast(tmpf(0,1,1,1,1),Nx*Nz*15*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        ! call fromtmpf(f_procs)

        ! if(new_rank == 0) then
        !     call totmpf(geq)
        ! endif
        ! call MPI_Bcast(tmpf(0,1,1,1,1),Nx*Nz*15*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        ! call fromtmpf(g_procs)
        ! write(*,*) "8"
        ...
        call MPI_Bcast(tmp1(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        call MPI_Bcast(tmp2(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        call MPI_Bcast(tmp3(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        call MPI_Bcast(tmp4(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        call MPI_Bcast(tmp5(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        call MPI_Bcast(tmpf(0,1,1,1,1),Nx*Nz*15*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        call MPI_Bcast(tmpff(0,1,1,1,1),Nx*Nz*15*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
    ...
    subroutine fromtmp(fun_procs)
        ...
        enddo
    endsubroutine fromtmp

    subroutine totmpf(fun)
    ...
    endsubroutine totmpf

    subroutine fromtmpf(fun_procs)
    ...
    subroutine output(phi)
        ...
        do zi=0,zmax
            do yi=0,ymax
                do xi=0,xmax
                    write(20,"(4es24.16)") dble(xi),dble(yi),dble(zi),phi(xi,yi,zi)
                enddo
                write(20,*)
            enddo
        enddo
