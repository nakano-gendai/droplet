module globals
    ...
    integer,parameter:: start = 500000 !壁を動かす時間step
    integer i, n, xi, yi, zi, alpha, beta, k
    character :: filename*20
    character(*),parameter :: datadir = "./data_c/"
...
contains
    ...
    subroutine ini(phi,u1,u2,u3,p,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs)
        ...
        real(8),allocatable ::  feq_procs(:,:,:,:), geq_procs(:,:,:,:)
        ...
        !     do yi=1,ymax+1
        !         do xi=1,x_procs
        !             write(60, *) xi,yi,zi,phi_procs(xi,yi,zi)
        !         enddo
        !     enddo
        ! enddo
        ! close(60)
    ...
    subroutine ini_op(Anu_procs,nu_procs,p0_procs,lap_phi_procs,grad_phi_procs,gphi_procs,pcap_procs,div_pcap_procs,grad_u_procs,phi_procs)
        real(8),intent(in):: phi_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),allocatable :: Anu_procs(:,:,:), nu_procs(:,:,:)
        real(8),allocatable :: p0_procs(:,:,:), lap_phi_procs(:,:,:), grad_phi_procs(:,:,:,:), gphi_procs(:,:,:,:,:)
        real(8),allocatable :: pcap_procs(:,:,:,:,:), div_pcap_procs(:,:,:,:), grad_u_procs(:,:,:,:,:)
        !以下はのりしろ無しの変数
        allocate(Anu_procs(1:x_procs,1:ymax+1,1:y_procs))
        ...
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    ...
                    Anu_procs(xi,yi,zi) = 4.5d0*(1.0d0/6.0d0-nu_procs(xi,yi,zi)/ds)
                enddo
            enddo
        enddo
    ...
    subroutine MPI_boundary(phi_procs,p_procs,u1_procs,u2_procs,u3_procs)
    ...
    subroutine geq_cal(gphi_procs,grad_u_procs,p_procs,u1_procs,u2_procs,u3_procs,Anu_procs,geq_procs,cr)
        real(8),intent(in):: gphi_procs(1:3,1:3,1:x_procs,1:ymax+1,1:y_procs)
        ...
        real(8),intent(in):: u2_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(in):: u3_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(in):: Anu_procs(1:x_procs,1:ymax+1,1:y_procs)
    ...
    subroutine bounce_back(fun,U)
        ...
        if(group_y == 0) then
            ...
            do yi=1,ymax+1
                do xi=1,x_procs
                    fun(4,xi-cx(4),yi-cy(4),zi-cz(4)) = fun(7,xi-cx(7),yi-cy(7),zi-cz(7)) - 6.0d0 * E(7) * ((-U(1))*cr(1,7)+(-U(2))*cr(2,7)+(-U(3))*cr(3,7))
                    fun(8,xi-cx(8),yi-cy(8),zi-cz(8)) = fun(12,xi-cx(12),yi-cy(12),zi-cz(12)) - 6.0d0 * E(12) * ((-U(1))*cr(1,12)+(-U(2))*cr(2,12)+(-U(3))*cr(3,12))
                    fun(9,xi-cx(9),yi-cy(9),zi-cz(9)) = fun(13,xi-cx(13),yi-cy(13),zi-cz(13)) - 6.0d0 * E(13) * ((-U(1))*cr(1,13)+(-U(2))*cr(2,13)+(-U(3))*cr(3,13))
                    fun(10,xi-cx(10),yi-cy(10),zi-cz(10)) = fun(14,xi-cx(14),yi-cy(14),zi-cz(14)) - 6.0d0 * E(14) * ((-U(1))*cr(1,14)+(-U(2))*cr(2,14)+(-U(3))*cr(3,14))
                    fun(15,xi-cx(15),yi-cy(15),zi-cz(15)) = fun(11,xi-cx(11),yi-cy(11),zi-cz(11)) - 6.0d0 * E(11) * ((-U(1))*cr(1,11)+(-U(2))*cr(2,11)+(-U(3))*cr(3,11))
                enddo
            enddo
            ...
            do yi=1,ymax+1
                do xi=1,x_procs
                    fun(7,xi-cx(7),yi-cy(7),zi-cz(7)) = fun(4,xi-cx(4),yi-cy(4),zi-cz(4)) - 6.0d0 * E(4) * ((U(1))*cr(1,4)+(U(2))*cr(2,4)+(U(3))*cr(3,4))
                    fun(11,xi-cx(11),yi-cy(11),zi-cz(11)) = fun(15,xi-cx(15),yi-cy(15),zi-cz(15)) - 6.0d0 * E(15) * ((U(1))*cr(1,15)+(U(2))*cr(2,15)+(U(3))*cr(3,15))
                    fun(12,xi-cx(12),yi-cy(12),zi-cz(12)) = fun(8,xi-cx(8),yi-cy(8),zi-cz(8)) - 6.0d0 * E(8) * ((U(1))*cr(1,8)+(U(2))*cr(2,8)+(U(3))*cr(3,8))
                    fun(13,xi-cx(13),yi-cy(13),zi-cz(13)) = fun(9,xi-cx(9),yi-cy(9),zi-cz(9)) - 6.0d0 * E(9) * ((U(1))*cr(1,9)+(U(2))*cr(2,9)+(U(3))*cr(3,9))
                    fun(14,xi-cx(14),yi-cy(14),zi-cz(14)) = fun(10,xi-cx(10),yi-cy(10),zi-cz(10)) - 6.0d0 * E(10) * ((U(1))*cr(1,10)+(U(2))*cr(2,10)+(U(3))*cr(3,10))
                enddo
            enddo
        endif
    end subroutine bounce_back

    subroutine physics(phi_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,geq_procs,nu_procs,Anu_procs)
        real(8),intent(inout):: phi_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        ...
        real(8),intent(inout):: Anu_procs(1:x_procs,1:ymax+1,1:y_procs)
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    ...
                    do i=1,15
                        ...
                        p_procs(xi,yi,zi) = p_procs(xi,yi,zi) + geq_procs(i,xi-cx(i),yi-cy(i),zi-cz(i)) / 3.0d0
                        u1_procs(xi,yi,zi) = u1_procs(xi,yi,zi) + dble(cx(i))*geq_procs(i,xi-cx(i),yi-cy(i),zi-cz(i))
                        u2_procs(xi,yi,zi) = u2_procs(xi,yi,zi) + dble(cy(i))*geq_procs(i,xi-cx(i),yi-cy(i),zi-cz(i))
                        u3_procs(xi,yi,zi) = u3_procs(xi,yi,zi) + dble(cz(i))*geq_procs(i,xi-cx(i),yi-cy(i),zi-cz(i))
                    enddo
                    nu_procs(xi,yi,zi) = 0.5d0*(nu2-nu1)*(dsin(pi*(phi_procs(xi,yi,zi)-0.5d0*(phi2+phi1))/(phi2-phi1)) + 1.d0) + nu1
                    Anu_procs(xi,yi,zi) = 4.5d0*(1.0d0/6.0d0-nu_procs(xi,yi,zi)/ds)
                enddo
    ...
    subroutine grad_u_cal(grad_u,u1,u2,u3)
        real(8),intent(out):: grad_u(1:3,1:3,1:x_procs,1:ymax+1,1:y_procs)
        real(8),intent(in):: u1(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(in):: u2(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(in):: u3(0:x_procs+1,0:ymax+2,0:y_procs+1)
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    do beta=1,3
    ...
    subroutine laplace(phi,p)
        ...
        write(8,"(7es16.8)") dble(n),dp_cal,veff,reff,sigma,kappag
...
use globals
    implicit none
    real(8) feq(1:15, 0:xmax,0:ymax,0:zmax) !局所平衡分布関数（phi計算用）
    real(8) geq(1:15, 0:xmax,0:ymax,0:zmax) !局所平衡分布関数（速度・圧力計算用）
    real(8) phi(0:xmax,0:ymax,0:zmax) !秩序パラメータ
    ...
    call ini(phi,u1,u2,u3,p,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs)
    call ini_op(Anu_procs,nu_procs,p0_procs,lap_phi_procs,grad_phi_procs,gphi_procs,pcap_procs,div_pcap_procs,grad_u_procs,phi_procs)
...
DO n=1,step
!======================のりしろ境界==========================================================
    call MPI_boundary(phi_procs,p_procs,u1_procs,u2_procs,u3_procs)
!=======================div/gradの計算============================================================
...
!=======================平衡分布関数の計算========================================================
    ...
    call geq_cal(gphi_procs,grad_u_procs,p_procs,u1_procs,u2_procs,u3_procs,Anu_procs,geq_procs,cr)
    call MPI_boundary_fg(feq_procs,geq_procs)
    call bounce_back(feq_procs,U)
    call bounce_back(geq_procs,U)
!==========================物理量の計算==========================================================
    call physics(phi_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,geq_procs,nu_procs,Anu_procs)
!============================物理量をまとめる======================================================
    !p
    tmp(:,:,:) = 0.0d0
    do zi=1,y_procs
