! 【保存型 Allen-Cahn 方程式】
! このプログラムは、界面を「保存型 Allen-Cahn 方程式」で表現している。
! 液滴を長時間保持できるが、バルク領域での数値誤差の小さなゆらぎから
! 微小な液滴や気泡が発生したり、またそれらが非物理的な動きをすることがある。
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module globals
    include "mpif.h"
    ...
    ! character(*),parameter :: datadir = "/home/nakano/actest/data/"
    character(*),parameter :: datadir = "/data/2022/nakano/dropac4/"
    ...
    real(8),parameter:: nu1 = 0.2d0 !液体の動粘度
    real(8),parameter:: nu2 = 0.2d0 !液滴の動粘度
    real(8),parameter:: uw = 0.01d0 !壁の移動速度
    real(8),parameter:: kappaf = 0.01d0*ds**2 !界面厚さを決めるパラメータ
    real(8),parameter:: kappag = 8.132d-3 !界面張力を決めるパラメータ
    ...
    !その他変数
    real(8) phi_min, phi_max, sigma, sigma_th, wa, min, max
...
contains
    ...
    subroutine ini(phi,u1,u2,u3,p,geq,g_procs,gnext_procs,feq,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi,gphi,n_vector,grad_u)
    ...
    !===================================初期条件の設定================================================
        ...
        do zi=0,zmax
            do yi=0,ymax
                do xi=0,xmax
                    do alpha=1,3
                        n_vector(alpha,xi,yi,zi) = grad_phi(alpha,xi,yi,zi) / (sqrt(grad_phi(1,xi,yi,zi)**2 + grad_phi(2,xi,yi,zi)**2 + grad_phi(3,xi,yi,zi)**2) + eps)
                    enddo
                enddo
            enddo
        enddo
        ...
        do zi=0,zmax
            do yi=0,ymax
                do xi=0,xmax
                    do i=1,15
                        feq(i,xi,yi,zi) = E(i)*phi(xi,yi,zi)*(1.0d0+3.0d0*(cr(1,i)*u1(xi,yi,zi)+cr(2,i)*u2(xi,yi,zi)+cr(3,i)*u3(xi,yi,zi))) &
                                        +3.0d0*E(i)*M*(1.0d0-4.0d0*phi(xi,yi,zi)**2) &
                                        *(cr(1,i)*n_vector(1,xi,yi,zi)+cr(2,i)*n_vector(2,xi,yi,zi)+cr(3,i)*n_vector(3,xi,yi,zi))/W
                    enddo
                enddo
            enddo
        enddo
                    ...
                    do yi=1,ymax+1
                        do xi=1,x_procs
                            ...
                            do i=1,15
                                ...
                                feq_procs(i,xi,yi,zi) = tmpff(n,i,xi,yi,zi)
                            enddo
                        enddo
                    enddo
                enddo
    ...
    subroutine ini_op(taug_procs,nu_procs,n_vector_procs,phinext_procs,grad_phi_procs,gphi_procs,grad_u_procs,phi_procs)
        real(8),intent(in):: phi_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),allocatable :: taug_procs(:,:,:), nu_procs(:,:,:)
        real(8),allocatable :: n_vector_procs(:,:,:,:), phinext_procs(:,:,:), grad_phi_procs(:,:,:,:), gphi_procs(:,:,:,:,:), grad_u_procs(:,:,:,:,:)
        !以下はのりしろ無しの変数
        allocate(taug_procs(0:x_procs+1,0:ymax+2,0:y_procs+1))
        allocate(phinext_procs(0:x_procs+1,0:ymax+2,0:y_procs+1))
        allocate(nu_procs(1:x_procs,1:ymax+1,1:y_procs))
        allocate(n_vector_procs(1:3,1:x_procs,1:ymax+1,1:y_procs))
    ...
    subroutine MPI_boundary(phi_procs,u1_procs,u2_procs,u3_procs,g_procs,taug_procs)
    ...
    subroutine feq_cal(phi_procs,u1_procs,u2_procs,u3_procs,n_vector_procs,feq_procs)
        real(8),intent(in):: phi_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        ...
        real(8),intent(in):: n_vector_procs(1:3,1:x_procs,1:ymax+1,1:y_procs)
        real(8),intent(out):: feq_procs(1:15,0:x_procs+1,0:ymax+2,0:y_procs+1)
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    do i=1,15
                        feq_procs(i,xi,yi,zi) = E(i)*phi_procs(xi,yi,zi)*(1.0d0+3.0d0*(cr(1,i)*u1_procs(xi,yi,zi) &
                        +cr(2,i)*u2_procs(xi,yi,zi)+cr(3,i)*u3_procs(xi,yi,zi)))+3.0d0*E(i)*M*(1.0d0-4.0d0*phi_procs(xi,yi,zi)**2) &
                        *(cr(1,i)*n_vector_procs(1,xi,yi,zi)+cr(2,i)*n_vector_procs(2,xi,yi,zi)+cr(3,i)*n_vector_procs(3,xi,yi,zi))/W
                    enddo
                enddo
            enddo
        enddo
    ...
    subroutine physics(phi_procs,phinext_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,g_procs)
        real(8),intent(in):: phi_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(inout):: phinext_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        ...
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    phinext_procs(xi,yi,zi) = 0.0d0
                    p_procs(xi,yi,zi) = 0.0d0
                    u1_procs(xi,yi,zi) = 0.0d0
                    u2_procs(xi,yi,zi) = 0.0d0
                    u3_procs(xi,yi,zi) = 0.0d0
                    do i=1,15
                        phinext_procs(xi,yi,zi) = phinext_procs(xi,yi,zi) + feq_procs(i,xi-cx(i),yi-cy(i),zi-cz(i)) + A*E(i)*(phi_procs(xi,yi,zi)-phi_procs(xi-cx(i),yi-cy(i),zi-cz(i)))
    ...
    subroutine renew2(fun_procs,funnext_procs)
        real(8),intent(in) :: funnext_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        real(8),intent(out) :: fun_procs(0:x_procs+1,0:ymax+2,0:y_procs+1)
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    do i=1,15
                        fun_procs(xi,yi,zi) = funnext_procs(xi,yi,zi)
                    enddo
                enddo
            enddo
        enddo
    end subroutine renew2
    ...
    subroutine n_vector_cal(grad_phi_procs,n_vector_procs)
        real(8),intent(in):: grad_phi_procs(1:3,1:x_procs,1:ymax+1,1:y_procs)
        real(8),intent(out):: n_vector_procs(1:3,1:x_procs,1:ymax+1,1:y_procs)
        do zi=1,y_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    do alpha=1,3
                        n_vector_procs(alpha,xi,yi,zi) = grad_phi_procs(alpha,xi,yi,zi) &
                        / (sqrt(grad_phi_procs(1,xi,yi,zi)**2 + grad_phi_procs(2,xi,yi,zi)**2 + grad_phi_procs(3,xi,yi,zi)**2) + eps)
                    enddo
                enddo
            enddo
        enddo
    end subroutine n_vector_cal
    ...
    subroutine break_up(phi,p,grad_phi)
        ...
        if(n <= start) then
            ...
            write(8,"(5es16.8)") dble(n),L1,sigma,reff,sigma_th
        endif
...
use globals
    ...
    call ini(phi,u1,u2,u3,p,geq,g_procs,gnext_procs,feq,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi,gphi,n_vector,grad_u)
    call ini_op(taug_procs,nu_procs,n_vector_procs,phinext_procs,grad_phi_procs,gphi_procs,grad_u_procs,phi_procs)
...
DO n=1,step
    kappag_tmp = kappag
!======================のりしろ境界==========================================================
    call MPI_boundary(phi_procs,u1_procs,u2_procs,u3_procs,g_procs,taug_procs)
!=======================変数の計算============================================================
    call grad_cal(grad_phi_procs,phi_procs)
    call n_vector_cal(grad_phi_procs,n_vector_procs)
    call gphi_cal(gphi_procs,grad_phi_procs)
    call grad_u_cal(grad_u_procs,u1_procs,u2_procs,u3_procs)
!=======================局所平衡分布関数の計算========================================================
    call feq_cal(phi_procs,u1_procs,u2_procs,u3_procs,n_vector_procs,feq_procs)
...
!==========================物理量の計算==========================================================
    call physics(phi_procs,phinext_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,g_procs)
    call renew2(phi_procs,phinext_procs)
    min = phi2
        max = phi1
        do zi=1,y_procs
            do yi=1,ymax+1
