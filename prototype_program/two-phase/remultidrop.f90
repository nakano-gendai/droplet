...
module globals
    ...
    integer,parameter:: xmax = 999 !ｘ方向格子数（０から数える）
    integer,parameter:: ymax = 290 !ｙ方向格子数（０から数える）
    integer,parameter:: zmax = 290 !ｚ方向格子数（０から数える）
    ...
    ! character(*),parameter :: datadir = "/data/sht/nakanog/re150_r55_h440/"
    ! character(*),parameter :: datadir2 = "/data/sht/nakanog/re150_r55_h440/fg/"
    character(*),parameter :: datadir = "/data/n/n517/re200we7_d-2/"
    character(*),parameter :: datadir2 = "/data/n/n517/re200we7_d-2/fg/"

    !無次元数
    real(8),parameter:: We = 7.0d0 !粒子ウエーバー数
    real(8),parameter:: Re = 200.0d0 !粒子レイノルズ数
...
contains
    ...
    subroutine ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,taug_procs,nu_procs)
    ...
    !========================並列数・コミュニケータ分割・通信先設定========================
        ...
        Nx = 25 !x方向の並列数（ただし，Nx/=comm_procs）
    ...
    !===============================================================================================================
        ...
        filename=datadir2//trim(adjustl(filename))//'_'//trim(adjustl(filename3))//'_100000_fg.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
        ...
        do zi=1,z_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    do i=1,15
                        read(100) f_procs(i,xi,yi,zi),g_procs(i,xi,yi,zi)
                    enddo
                enddo
            enddo
        enddo
    ...
    subroutine output(phi_procs,u1_procs,u2_procs,u3_procs,p_procs)
        ...
        do zi=1,z_procs
            ! do yi=1,ymax+1
                do xi=1,x_procs
                    write(101) real(phi_procs(xi,yi,zi)),real(u1_procs(xi,yi,zi)),real(u2_procs(xi,yi,zi)),real(u3_procs(xi,yi,zi)),real(p_procs(xi,yi,zi))
                    ! write(20) real(xi),real(zi),real(phi_procs(xi,yi,zi))
                enddo
                write(101)
            ! enddo
        enddo
...
use globals
    ...
    call ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,taug_procs,nu_procs)
...
!==========================物理量の計算==========================================================
    ...
    if(comm_rank == 0) then
        ...
        if(mod(n,100)==0) then
            if(n == 100100) then
                open(10,file="./time.d",status='replace')
