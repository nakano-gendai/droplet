...
module globals
    ...
    !時間に関するパラメータ
    integer,parameter:: step = 1000 !計算時間step
    !出力ディレクトリ
    ! character(*),parameter :: datadir = "/data/n/n517/mpi/"
    ! character(*),parameter :: datadir2 = "/data/n/n517/mpi/fg/"
    character(*),parameter :: datadir = "/data/sht/nakanog/mpi/"
    character(*),parameter :: datadir2 = "/data/sht/nakanog/mpi/fg/"

    !無次元数
    real(8),parameter:: We = 3.0d0 !粒子ウエーバー数
    real(8),parameter:: Re = 100.0d0 !粒子レイノルズ数
...
contains
    ...
    subroutine ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,forcex,forcey,forcez,x_m,y_m,eps,eps2)
...
!================================乱数の発生（標準正規分布）=====================================
    ...
    subroutine ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs)
...
!================================初期条件・配列のallocate======================================
    ...
    call ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,forcex,forcey,forcez,x_m,y_m,eps,eps2)
    call ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs)
...
!================================出力==================================
    ...
    if(comm_rank == 0) then
        if(mod(n,1)==0) then
            if(n == 1) then
                open(12,file="./time_mpi.d")
                ! write(10,*) n, time2-time1, u1_procs(1,1,1), u2_procs(1,1,1), u3_procs(1,1,1)
                write(12,*) n, time2-time1, g_procs(1,1,1,1), g_procs(2,1,1,1), g_procs(3,1,1,1)
                close(12)
            else
                open(12,file="./time_mpi.d",action="write",position="append")
                ! write(10,*) n, time2-time1, u1_procs(1,1,1), u2_procs(1,1,1), u3_procs(1,1,1)
                write(12,*) n, time2-time1, g_procs(1,1,1,1), g_procs(2,1,1,1), g_procs(3,1,1,1)
                close(12)
            endif
