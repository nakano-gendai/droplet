...
module globals
    ...
    !出力ディレクトリ
    ! character(*),parameter :: datadir = "/data/n/n517/taylor_re7000_255/"
    ! character(*),parameter :: datadir2 = "/data/n/n517/taylor_re7000_255/fg/"
    character(*),parameter :: datadir = "/data/sht/nakanog/taylor_re2000_ran/"
    character(*),parameter :: datadir2 = "/data/sht/nakanog/taylor_re2000_ran/fg/"

    ! character(*),parameter :: datadir_ran = "/data/n/n517/taylor_re1000_255/ran/"
...
contains
    ...
    subroutine ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,forcex,forcey,forcez,taug_procs,nu_procs,x_m,y_m,eps,eps2)
...
!================================乱数の発生（標準正規分布）=====================================
        ...
        filename=datadir2//trim(adjustl(filename))//'_'//trim(adjustl(filename3))//'_484000_fg.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
...
!================================初期条件・配列のallocate======================================
    ...
    call ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,forcex,forcey,forcez,taug_procs,nu_procs,x_m,y_m,eps,eps2)
...
!================================出力==================================
    ...
    if(mod(n,4000)==0) then
        ...
        if(comm_rank == 0) then
            ...
            if(n == 4000) then
                open(11,file="./kinetic_re2000_ran.d")
                write(11,*) dble(n), kinetic_ave
                close(11)
            else
                open(11,file="./kinetic_re2000_ran.d",action="write",position="append")
                write(11,*) dble(n), kinetic_ave
                close(11)
            endif
        endif
