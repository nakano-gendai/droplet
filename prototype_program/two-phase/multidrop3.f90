...
module globals
    ...
    integer,parameter:: xmax = 9 !ｘ方向格子数（０から数える）
    integer,parameter:: ymax = 9 !ｙ方向格子数（０から数える）
    integer,parameter:: zmax = 9 !ｚ方向格子数（０から数える）
    ...
    character(*),parameter :: datadir = "/data/n/n517/testplay10/"
...
contains
    ...
    subroutine ini(phi,u1,u2,u3,p,geq,g_procs,gnext_procs,feq,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi,gphi,lap_phi,p0)
        ...
        real(8),allocatable :: geq(:,:,:,:)
        real(8),allocatable :: grad_phi(:,:,:,:)
        real(8),allocatable :: lap_phi(:,:,:)
        real(8),allocatable :: gphi(:,:,:,:,:)
    ...
    !===============================================================================================================
        ! new_rank = 0 だけ配列を設定
        if(new_rank == 0) then
            allocate(phi(-1:xmax+1,-1:ymax+1,-1:zmax+1))
            allocate(u1(-1:xmax+1,-1:ymax+1,-1:zmax+1))
            allocate(u2(-1:xmax+1,-1:ymax+1,-1:zmax+1))
            allocate(u3(-1:xmax+1,-1:ymax+1,-1:zmax+1))
            allocate(p(-1:xmax+1,-1:ymax+1,-1:zmax+1))
            allocate(feq(1:15,0:xmax,0:ymax,0:zmax))
            allocate(geq(1:15,0:xmax,0:ymax,0:zmax))
            allocate(grad_phi(1:3,0:xmax,0:ymax,0:zmax))
            allocate(lap_phi(0:xmax,0:ymax,0:zmax))
            allocate(gphi(1:3,1:3,0:xmax,0:ymax,0:zmax))
            allocate(p0(0:xmax,0:ymax,0:zmax))
        endif
        !以下はのりしろ有りの変数
        ...
        call MPI_Bcast(tmp1(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        do n = 0, new_procs-1
            if(n == new_rank) then
                do zi=1,z_procs
                    do yi=1,ymax+1
            ...
            do Nzz=0,Nz-1
                do Nxx=0,Nx-1
                    do zi=1,z_procs
                        do yi=1,ymax+1
                            do xi=1,x_procs
                                tmp1(nstar,xi,yi,zi) = u2((xi-1)+Nxx*x_procs,yi-1,(zi-1)+Nzz*(z_procs))
                            enddo
                        enddo
                    enddo
                    nstar = nstar + 1
        ...
        do n = 0, new_procs-1
            if(n == new_rank) then
                do zi=1,z_procs
                    do yi=1,ymax+1
                        do xi=1,x_procs
                            u2_procs(xi,yi,zi) = tmp1(n,xi,yi,zi)
                        enddo
                    enddo
                enddo
            endif
            ...
            do Nzz=0,Nz-1
                do Nxx=0,Nx-1
                    do zi=1,z_procs
                        do yi=1,ymax+1
                            do xi=1,x_procs
                                tmp1(nstar,xi,yi,zi) = u3((xi-1)+Nxx*x_procs,yi-1,(zi-1)+Nzz*(z_procs))
                            enddo
                        enddo
                    enddo
                    nstar = nstar + 1
        ...
        do n = 0, new_procs-1
            if(n == new_rank) then
                do zi=1,z_procs
                    do yi=1,ymax+1
                        do xi=1,x_procs
                            u3_procs(xi,yi,zi) = tmp1(n,xi,yi,zi)
                        enddo
                    enddo
                enddo
            endif
            ...
            do Nzz=0,Nz-1
                do Nxx=0,Nx-1
                    do zi=1,z_procs
                        do yi=1,ymax+1
                            do xi=1,x_procs
                                tmp1(nstar,xi,yi,zi) = p((xi-1)+Nxx*x_procs,yi-1,(zi-1)+Nzz*(z_procs))
                            enddo
                        enddo
                    enddo
                    nstar = nstar + 1
        ...
        do n = 0, new_procs-1
            if(n == new_rank) then
                do zi=1,z_procs
                    do yi=1,ymax+1
                        do xi=1,x_procs
                            p_procs(xi,yi,zi) = tmp1(n,xi,yi,zi)
                        enddo
                    enddo
                enddo
            endif
            ...
            do Nzz=0,Nz-1
                do Nxx=0,Nx-1
                    do zi=1,z_procs
                        do yi=1,ymax+1
                            do xi=1,x_procs
                                tmp1(nstar,xi,yi,zi) = phi((xi-1)+Nxx*x_procs,yi-1,(zi-1)+Nzz*(z_procs))
                            enddo
                        enddo
                    enddo
                    nstar = nstar + 1
        ...
        do n = 0, new_procs-1
            if(n == new_rank) then
                do zi=1,z_procs
                    do yi=1,ymax+1
                        do xi=1,x_procs
                            phi_procs(xi,yi,zi) = tmp1(n,xi,yi,zi)
                        enddo
                    enddo
                enddo
            endif
        ...
        call MPI_Bcast(tmpf(0,1,1,1,1),Nx*Nz*15*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,new_comm_world,ierr)
        do n = 0, new_procs-1
            if(n == new_rank) then
                do zi=1,z_procs
                    do yi=1,ymax+1
                ...
                do Nxx=0,Nx-1
                    do zi=1,z_procs
                        do yi=1,ymax+1
                            do xi=1,x_procs
                                do i=1,15
                                    tmpf(nstar,i,xi,yi,zi) = feq(i,(xi-1)+Nxx*x_procs,yi-1,(zi-1)+Nzz*(z_procs))
                                enddo
                            enddo
                        enddo
                    enddo
    ...
    subroutine ini_op(taug_procs,nu_procs,p0_procs,lap_phi_procs,grad_phi_procs,gphi_procs,phi_procs)
    ...
    subroutine output(phi,u1,u2,u3,p)
        ...
        do zi=0,zmax
            do yi=0,ymax
                do xi=0,xmax
                    write(20,"(8es24.16)") dble(xi),dble(yi),dble(zi),phi(xi,yi,zi),u1(xi,yi,zi),u2(xi,yi,zi),u3(xi,yi,zi),p(xi,yi,zi)
                enddo
                ! write(20,*)
            enddo
        enddo
    ...
    subroutine outputfg(feq,geq)
        real(8),intent(inout):: feq(1:15,0:xmax,0:ymax,0:zmax)
        real(8),intent(inout):: geq(1:15,0:xmax,0:ymax,0:zmax)

        write(filename,*) group_new !i->filename 変換
        write(filename2,*) n
        filename=datadir//trim(adjustl(filename))//'_'//trim(adjustl(filename2))//'_fg.d' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
        print *, filename !表示してみる
        ! open(20,file=filename, form='unformatted',status='replace') 
        open(20,file=filename,status='replace') 
        ! yi = ymax / 2 
        do zi=0,zmax
            do yi=0,ymax
                do xi=0,xmax
                    do i=1,15
                        write(20,"(5es24.16)") dble(xi),dble(yi),dble(zi),feq(i,xi,yi,zi),geq(i,xi,yi,zi)
                    enddo
                enddo
                ! write(20,*)
            enddo
...
use globals
    implicit none
    real(8),allocatable :: feq(:,:,:,:) !平衡分布関数（オーダーパラメータ）
    real(8),allocatable :: geq(:,:,:,:) !平衡分布関数（速度・圧力計算用）
    real(8),allocatable :: phi(:,:,:) !秩序パラメータ
    real(8),allocatable :: u1(:,:,:), u2(:,:,:), u3(:,:,:) !流速
    real(8),allocatable :: p(:,:,:) !圧力
    real(8),allocatable :: grad_phi(:,:,:,:)
    real(8),allocatable :: lap_phi(:,:,:)
    real(8),allocatable :: gphi(:,:,:,:,:)
    real(8),allocatable :: p0(:,:,:)
    ...
    real(8),allocatable :: phiout(:,:,:,:), u1out(:,:,:,:), u2out(:,:,:,:), u3out(:,:,:,:), pout(:,:,:,:)
    real(8),allocatable :: fout(:,:,:,:,:), gout(:,:,:,:,:)
    ...
    call ini(phi,u1,u2,u3,p,geq,g_procs,gnext_procs,feq,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi,gphi,lap_phi,p0)
    call ini_op(taug_procs,nu_procs,p0_procs,lap_phi_procs,grad_phi_procs,gphi_procs,phi_procs)
    
    if(new_rank == 0) then
        allocate(phiout(0:x_procs+1,0:ymax+2,0:z_procs+1,0:new_procs-1))
        allocate(u1out(0:x_procs+1,0:ymax+2,0:z_procs+1,0:new_procs-1))
        allocate(u2out(0:x_procs+1,0:ymax+2,0:z_procs+1,0:new_procs-1))
        allocate(u3out(0:x_procs+1,0:ymax+2,0:z_procs+1,0:new_procs-1))
        allocate(pout(0:x_procs+1,0:ymax+2,0:z_procs+1,0:new_procs-1))
        allocate(fout(1:15,0:x_procs+1,0:ymax+2,0:z_procs+1,0:new_procs-1))
        allocate(gout(1:15,0:x_procs+1,0:ymax+2,0:z_procs+1,0:new_procs-1))
    endif
...
!==========================物理量の計算==========================================================
    ...
    if( mod(n,500)==0 ) then
        ...
        do i=1,new_procs-1
            if(new_rank == i) then
                tags = i
                call MPI_Isend(phi_procs(0,0,0),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,0,tags,new_comm_world,req1s,ierr)
                call MPI_Wait(req1s,sta1s,ierr)
            endif
            if(new_rank == 0) then
                tagr = i
                recv_rank = i
                call MPI_Irecv(phiout(0,0,0,i),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,recv_rank,tagr,new_comm_world,req1r,ierr)
                call MPI_Wait(req1r,sta1r,ierr)
            endif
        enddo
        if(new_rank == 0) then
        ...
        do i=1,new_procs-1
            if(new_rank == i) then
                tags = i
                call MPI_Isend(u1_procs(0,0,0),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,0,tags,new_comm_world,req2s,ierr)
                call MPI_Wait(req2s,sta2s,ierr)
            endif
            if(new_rank == 0) then
                tagr = i
                recv_rank = i
                call MPI_Irecv(u1out(0,0,0,i),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,recv_rank,tagr,new_comm_world,req2r,ierr)
                call MPI_Wait(req2r,sta2r,ierr)
            endif
        enddo
        if(new_rank == 0) then
            do zi=0,z_procs+1
                do yi=0,ymax+2
                    do xi=0,x_procs+1
                        u1out(xi,yi,zi,0) = u1_procs(xi,yi,zi)
                    enddo
                enddo
            enddo
        endif
        ...
        do i=1,new_procs-1
            if(new_rank == i) then
                tags = i
                call MPI_Isend(u2_procs(0,0,0),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,0,tags,new_comm_world,req3s,ierr)
                call MPI_Wait(req3s,sta3s,ierr)
            endif
            if(new_rank == 0) then
                tagr = i
                recv_rank = i
                call MPI_Irecv(u2out(0,0,0,i),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,recv_rank,tagr,new_comm_world,req3r,ierr)
        ...
        if(new_rank == 0) then
            do zi=0,z_procs+1
                do yi=0,ymax+2
                    do xi=0,x_procs+1
                        u2out(xi,yi,zi,0) = u2_procs(xi,yi,zi)
                    enddo
                enddo
            enddo
        endif
        ...
        do i=1,new_procs-1
            if(new_rank == i) then
                tags = i
                call MPI_Isend(u3_procs(0,0,0),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,0,tags,new_comm_world,req4s,ierr)
                call MPI_Wait(req4s,sta4s,ierr)
            endif
            if(new_rank == 0) then
                tagr = i
                recv_rank = i
                call MPI_Irecv(u3out(0,0,0,i),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,recv_rank,tagr,new_comm_world,req4r,ierr)
        ...
        if(new_rank == 0) then
            do zi=0,z_procs+1
                do yi=0,ymax+2
                    do xi=0,x_procs+1
                        u3out(xi,yi,zi,0) = u3_procs(xi,yi,zi)
                    enddo
                enddo
            enddo
        endif
        ...
        do i=1,new_procs-1
            if(new_rank == i) then
                tags = i
                call MPI_Isend(p_procs(0,0,0),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,0,tags,new_comm_world,req5s,ierr)
                call MPI_Wait(req5s,sta5s,ierr)
            endif
            if(new_rank == 0) then
                tagr = i
                recv_rank = i
                call MPI_Irecv(pout(0,0,0,i),(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,recv_rank,tagr,new_comm_world,req5r,ierr)
        ...
        if(new_rank == 0) then
            do zi=0,z_procs+1
                do yi=0,ymax+2
                    do xi=0,x_procs+1
                        pout(xi,yi,zi,0) = p_procs(xi,yi,zi)
                    enddo
                enddo
            enddo
        endif
        ...
        do i=1,new_procs-1
            if(new_rank == i) then
                tags = i
                call MPI_Isend(f_procs(1,0,0,0),15*(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,0,tags,new_comm_world,req6s,ierr)
                call MPI_Wait(req6s,sta6s,ierr)
            endif
            if(new_rank == 0) then
                tagr = i
                recv_rank = i
                call MPI_Irecv(fout(1,0,0,0,i),15*(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,recv_rank,tagr,new_comm_world,req6r,ierr)
        ...
        if(new_rank == 0) then
            do zi=0,z_procs+1
                do yi=0,ymax+2
                    do xi=0,x_procs+1
                        do i=1,15
                            fout(i,xi,yi,zi,0) = f_procs(i,xi,yi,zi)
                        enddo
                    enddo
                enddo
            enddo
        ...
        do i=1,new_procs-1
            if(new_rank == i) then
                tags = i
                call MPI_Isend(g_procs(1,0,0,0),15*(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,0,tags,new_comm_world,req7s,ierr)
                call MPI_Wait(req7s,sta7s,ierr)
            endif
            if(new_rank == 0) then
                tagr = i
                recv_rank = i
                call MPI_Irecv(gout(1,0,0,0,i),15*(x_procs+2)*(z_procs+2)*(ymax+3),MPI_REAL8,recv_rank,tagr,new_comm_world,req7r,ierr)
        ...
        if(new_rank == 0) then
            do zi=0,z_procs+1
                do yi=0,ymax+2
                    do xi=0,x_procs+1
                        do i=1,15
                            gout(i,xi,yi,zi,0) = g_procs(i,xi,yi,zi)
                        enddo
                    enddo
                enddo
            enddo
        ...
        if(new_rank == 0) then
                    ...
                    do zi=1,z_procs
                        do yi=1,ymax+1
                            do xi=1,x_procs
                                ...
                                do i=1,15
                                    feq(i,(xi-1)+Nxx*x_procs,yi-1,(zi-1)+Nzz*z_procs) = fout(i,xi,yi,zi,k)
                                    geq(i,(xi-1)+Nxx*x_procs,yi-1,(zi-1)+Nzz*z_procs) = gout(i,xi,yi,zi,k)
                                enddo
                            enddo
                        enddo
                    enddo
            ...
            call outputfg(feq,geq)
            
            ! if(n == 500) then
            !     write(chmyrank, "(i2.2)") group_new
            !     open(10,file="./laplace_"//chmyrank//".d")
            !     write(10,*) group_new, n, sigma
            !     close(10)
            ! else
            write(chmyrank, "(i2.2)") group_new
            open(10,file="./laplace_"//chmyrank//".d",action="write",position="append")
            write(10,*) group_new, n, sigma
            close(10)
            ! endif
        endif
