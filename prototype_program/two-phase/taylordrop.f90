...
module globals
    ...
    !出力ディレクトリ
    character(*),parameter :: datadir = "/data/n/n517/taylordrop_re1000_ran_0.5d_we50/"
    character(*),parameter :: datadir2 = "/data/n/n517/taylordrop_re1000_ran_0.5d_we50/fg/"
    ! character(*),parameter :: datadir = "/data/sht/nakanog/taylor_re1000we3/"
    ! character(*),parameter :: datadir2 = "/data/sht/nakanog/taylor_re1000we3/fg/"
    character(*),parameter :: datadir_ran = "/data/n/n517/re500we7_box_d-5/ran/"
    !入力ディレクトリ
    character(*),parameter :: datadir_ini = "/data/n/n517/taylor_re1000_255_ran/fg/"

    !無次元数
    real(8),parameter:: We = 50.0d0 !粒子ウエーバー数
    real(8),parameter:: Re = 1000.0d0 !粒子レイノルズ数
    real(8),parameter:: eta = 1.0d0 !粘度比（nu2/nu1）

    !撹乱（乱数）のオーダー
    real(8), parameter :: ran = 1.0d-5

    !支配パラメータ
    real(8),parameter:: pi = acos(-1.0d0) !円周率
    real(8),parameter:: D = 63.75d0 !設置する液滴直径
    ...
    integer i, j, k, n, nall, xi, yi, zi, alpha, beta
...
contains
    ...
    subroutine ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,forcex,forcey,forcez)
    ...
    subroutine ini_op(taug_procs,nu_procs,phi_procs,x_m,y_m,eps,eps2)
        real(8),intent(in):: phi_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),allocatable :: taug_procs(:,:,:), nu_procs(:,:,:), x_m(:,:,:,:), y_m(:,:,:,:), eps(:,:,:,:), eps2(:,:,:,:)
    ...
    subroutine g_cal(gnext_procs,g_procs,geq_procs,taug_procs,forcex,forcey,forcez)
                ...
                do xi=1,x_procs
                    do i=1,15
                        gnext_procs(i,xi,yi,zi) = g_procs(i,xi-cx(i),yi-cy(i),zi-cz(i)) &
                                                ...
                                                + 3.0d0 * ds * E(i) * (dble(cx(i))*forcex(xi-cx(i),yi-cy(i),zi-cz(i)) &
    ...
    subroutine externalforce(forcex,forcey,forcez)
        ...
        !             grobalx = (xi-1) + group_x * x_procs
        !             grobalz = (zi-1) + group_z * z_procs
        !             if((dble(grobalx) < ((dble(xmax)*0.5d0)-D_vortex)) .or. (dble(grobalx) > ((dble(xmax)*0.5d0)+D_vortex)) .or. (dble(grobalz) < ((dble(zmax)*0.5d0)-D_vortex)) .or. (dble(grobalz) > ((dble(zmax)*0.5d0)+D_vortex))) then
        !                 forcex(xi,yi,zi) = 0.0d0
        !                 forcey(xi,yi,zi) = 0.0d0
        !                 forcez(xi,yi,zi) = 0.0d0
        !             else
        !                 forcex(xi,yi,zi) = 2.0d0 * pi * pi * umax * umax / (D_vortex * Re) * sin(kw*(dble(grobalx)-(dble(xmax)*0.5d0-D_vortex)))  * cos(kw*(dble(grobalz)-(dble(zmax)*0.5d0-D_vortex)))
        !                 forcey(xi,yi,zi) = 0.0d0
        !                 forcez(xi,yi,zi) = -2.0d0 * pi * pi * umax * umax / (D_vortex * Re) * cos(kw*(dble(grobalx)-(dble(xmax)*0.5d0-D_vortex))) * sin(kw*(dble(grobalz)-(dble(zmax)*0.5d0-D_vortex)))
        !             endif
        !         enddo
        !     enddo
        ! enddo
        ...
        do zi=1,z_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    ...
                    forcex(xi,yi,zi) = 2.0d0 * pi * pi * umax * umax / (D_vortex * Re) * sin(kw*dble(grobalx))  * cos(kw*dble(grobalz))
                    forcey(xi,yi,zi) = 0.0d0
                    forcez(xi,yi,zi) = -2.0d0 * pi * pi * umax * umax / (D_vortex * Re) * cos(kw*dble(grobalx)) * sin(kw*dble(grobalz))
                enddo
            enddo
        enddo
    ...
    subroutine outputphi(phi_procs)
        ...
        write(filename2,*) n
        write(filename3,*) new_rank
        filename=datadir//trim(adjustl(filename))//'_'//trim(adjustl(filename3))//'_'//trim(adjustl(filename2))//'_phi.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
        print *, filename !表示してみる
...
!================================初期条件・配列のallocate======================================
    ...
    call ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,forcex,forcey,forcez)
    call ini_op(taug_procs,nu_procs,phi_procs,x_m,y_m,eps,eps2)
!================================乱数の発生（標準正規分布）=====================================
    ...
    !                 if(x_m(i,xi,yi,zi) > 3.0d0) then
    !                     x_m(i,xi,yi,zi) = 3.0d0
    !                 elseif(x_m(i,xi,yi,zi) < -3.0d0) then
    !                     x_m(i,xi,yi,zi) = -3.0d0
    !                 endif
    !             enddo
    !         enddo
    ...
    ! write(filename2,*) n
    ! write(filename3,*) new_rank
    ! filename=datadir_ran//trim(adjustl(filename))//'_'//trim(adjustl(filename3))//'_'//trim(adjustl(filename2))//'_ran.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
    ! print *, filename !表示してみる
    ! open(150,file=filename, form='unformatted',status='replace') 
    ! do zi=1,z_procs
    !     do yi=1,ymax+1
    !         do xi=1,x_procs
    !             do i=1,15
    !                 write(150) x_m(i,xi,yi,zi)
    !             enddo
    !         enddo
    !     enddo
    ! enddo
...
!============================流れの入力（撹乱を添加）=========================================
    if(n == 5000) then
        write(filename,*) group_new !i->filename 変換
        write(filename3,*) new_rank
        filename=datadir_ini//'0_'//trim(adjustl(filename3))//'_1000000_fg.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
        print *, filename !表示してみる
...
!=======================平衡分布関数gの計算=================================================
    ...
    call g_cal(gnext_procs,g_procs,geq_procs,taug_procs,forcex,forcey,forcez)
!=======================境界条件============================================================
