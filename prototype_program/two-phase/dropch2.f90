...
module globals
    ...
    integer,parameter:: xmax = 176 !ｘ方向格子数
    integer,parameter:: ymax = 176 !ｙ方向格子数
    integer,parameter:: zmax = 176 !ｚ方向格子数
    ...
    ! character(*),parameter :: datadir = "/data/n/n517/re100ca0.13/"
    ! character(*),parameter :: datadir = "/home/nakano/aomsin/data_aomsin/"
    character(*),parameter :: datadir = "/data/2022/nakano/crit/re100ca0.08_r44/"
    ...
    !支配パラメータ
    real(8),parameter:: D = 88.0d0 * ds !設置する液滴径
    real(8),parameter:: nu1 = 0.022d0 !液体の動粘度
    real(8),parameter:: nu2 = 0.022d0 !液滴の動粘度
    real(8),parameter:: uw = 0.1d0 !壁の移動速度
    real(8),parameter:: kappaf = 0.01d0*ds**2 !界面厚さを決めるパラメータ
    real(8),parameter:: kappag = 8.034d-3 !界面張力を決めるパラメータ
...
contains
    ...
    subroutine ini(phi,u1,u2,u3,p,geq,g_procs,gnext_procs,feq,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi,gphi,lap_phi,p0,grad_u)
        ...
        if(comm_rank == 0) then
            ...
            allocate(p(0:xmax,0:ymax,0:zmax))
    ...
    !===================================初期条件の設定================================================
        ...
        call MPI_Bcast(tmp1(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(tmp2(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(tmp3(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(tmp4(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(tmp5(0,1,1,1),Nx*Nz*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(tmpf(0,1,1,1,1),Nx*Nz*15*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        call MPI_Bcast(tmpff(0,1,1,1,1),Nx*Nz*15*x_procs*(ymax+1)*z_procs,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
        do n=0,comm_procs-1
            if(n == comm_rank) then
                do zi=1,y_procs
                    do yi=1,ymax+1
    ...
    subroutine break_up3(phi,p,grad_phi)
        ...
        if(n <= start) then
            ...
            write(8,"(4es16.8)") dble(n),L1,sigma,kappag_tmp
        endif
        ...
        do zi=0,zmax
            ! do yi=0,ymax
                do xi=0,xmax
                    ...
                    write(20,"(7es16.8)") dble(xi),dble(zi),phi(xi,yi,zi),u1(xi,yi,zi),u2(xi,yi,zi),u3(xi,yi,zi),p(xi,yi,zi)
                enddo
                write(20,*)
            ! enddo
        enddo
