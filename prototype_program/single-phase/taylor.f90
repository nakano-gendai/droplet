module globals
        ...
        integer,parameter:: xmax = 80 !ｘ方向格子数
        integer,parameter:: ymax = 80 !ｙ方向格子数
        ...
        real(8),parameter:: u0 = 0.005d0 !流速の初期値
        real(8),parameter:: p0 = rho / 3.0d0 !圧力の初期値
        real(8),parameter:: tau = 0.506d0
        real(8),parameter:: nu = (tau - 0.5d0)/3.0d0 !液体の動粘度
        real(8),parameter:: ep = 1.0d0 / tau
        real(8),parameter:: pi = acos(-1.0d0)
        real(8),parameter:: k1 = 2.0d0*pi/dble(xmax)
        real(8),parameter:: k2 = 2.0d0*pi/dble(ymax)
    ...
    contains
        ...
        subroutine initial(p,u1,u2,f)
            ...
            do yi=0,ymax
                do xi=0,xmax
                    p(xi,yi) = p0 +rho/4.0d0*(u0**2*cos(2.0d0*k1*dble(xi))+ &
                                k1**2/k2**2*u0**2*cos(2.0d0*k2*dble(yi)))
                    u1(xi,yi) = u0*sin(k1*dble(xi))*cos(k2*dble(yi))
                    u2(xi,yi) = -(k1/k2)*u0*cos(k1*dble(xi))*sin(k2*dble(yi))
                    do i =1,9
...
use globals
    ...
    real(8) forcex(-1:xmax+1, -1:ymax+1), forcey(-1:xmax+1, -1:ymax+1)
    call parv(cx,cy,cr)
    !初期値の設定
    call initial(p,u1,u2,f)
    do yi=0,ymax
        do xi=0,xmax
            forcex(xi,yi) = u0**2*2.0d0*pi**2/(40.0d0*100.0d0)*(sin(k1*dble(xi))*cos(k1*dble(yi)))
            forcey(xi,yi) = -u0**2*2.0d0*pi**2/(40.0d0*100.0d0)*(cos(k1*dble(xi))*sin(k1*dble(yi)))
        enddo
    enddo
    do yi=-1,ymax+1
        do xi=0,xmax
            forcex(xi,-1) = forcex(xi,ymax)
            forcey(xi,-1) = forcey(xi,ymax)
            forcex(xi,ymax+1) = forcex(xi,0)
            forcey(xi,ymax+1) = forcey(xi,0)
            forcex(-1,yi) = forcex(xmax,yi)
            forcey(-1,yi) = forcey(xmax,yi)
            forcex(xmax+1,yi) = forcex(0,yi)
            forcey(xmax+1,yi) = forcey(0,yi)
        enddo
    enddo
    open(100,file="./taylor.d")
    write(100,*) 0, u1(2,2),u2(2,2),p(2,2)
    close(100)
!===========時間発展=====================
    DO n=1,step
        !周期境界条件
            ...
            do xi=0,xmax
                do i=1,9
                    fnext(i,xi,yi) = f(i,xi-cx(i),yi-cy(i)) - ep * (f(i,xi-cx(i),yi-cy(i)) &
                        ...
                        + 3.0d0 * E(i) * (dble(cx(i))*forcex(xi,yi) + dble(cy(i))*forcey(xi,yi))
                enddo
            enddo
        enddo
        ! 速度分布関数の更新
        ...
        open(100,file="./taylor.d",action="write",position="append")
        write(100,*) n, u1(2,2),u2(2,2),p(2,2)
        close(100)
    ENDDO
    !出力
    ! call output(u1,u2,p)
