...
module globals
    ...
    ! character(*),parameter :: datadir = "/data/n/n517/re500we7_d-4/"
    ! character(*),parameter :: datadir2 = "/data/n/n517/re500we7_d-4/fg/"
    ! character(*),parameter :: datadir3 = "/data/n/n517/ini_0.05_999_290_290/fg/"

    integer klass(1000), klassnum
    ...
    integer rectangle_type_int,xtype_int
...
contains
    ...
    subroutine ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,eps,minus)
        ...
        real(8),allocatable :: p0_procs(:,:,:), lap_phi_procs(:,:,:), grad_phi_procs(:,:,:,:), gphi_procs(:,:,:,:,:), grad_u_procs(:,:,:,:,:),eps(:,:,:,:),minus(:,:,:,:)
    ...
    !========================並列数・コミュニケータ分割・通信先設定========================
        ...
        !x世界での受け渡しをする際の型作成
        call MPI_Type_Vector(ymax+3,x_procs,x_procs+2,MPI_INTEGER,xtype_int,ierr)
        call MPI_Type_Commit(xtype_int,ierr)

        !z世界での受け渡しをする際の型作成
        call MPI_Type_Vector((z_procs+2)*(ymax+3),1,x_procs+2,MPI_INTEGER,rectangle_type_int,ierr)
        call MPI_Type_Commit(rectangle_type_int,ierr)
    !===============================================================================================================
        !以下はのりしろ有りの変数
        ...
        allocate(eps(1:15,0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(minus(1:15,0:x_procs+1,0:ymax+2,0:z_procs+1))
    ...
    subroutine ini_op(taug_procs,nu_procs,phi_procs,label)
        ...
        allocate(label(0:x_procs+1,0:ymax+2,0:z_procs+1))
        !以下はのりしろ無しの変数
    ...
    subroutine glue_int(var)
        integer,intent(inout) :: var(0:x_procs+1,0:ymax+2,0:z_procs+1)
        do zi=1,z_procs
            do xi=1,x_procs
                var(xi,0,zi) = var(xi,ymax+1,zi)
                var(xi,ymax+2,zi) = var(xi,1,zi)
        ...
        enddo
        !X世界でののりしろ通信
        call MPI_Isend(var(1,0,z_procs),1,xtype_int,next_rank_x,1,newx_comm_world,req1s,ierr)
        call MPI_Irecv(var(1,0,0),1,xtype_int,former_rank_x,1,newx_comm_world,req1r,ierr)

        call MPI_Isend(var(1,0,1),1,xtype_int,former_rank_x,2,newx_comm_world,req2s,ierr)
        call MPI_Irecv(var(1,0,z_procs+1),1,xtype_int,next_rank_x,2,newx_comm_world,req2r,ierr)
        ...
        call MPI_Wait(req2r,sta2r,ierr)
        !Y世界でののりしろ通信
        call MPI_Isend(var(x_procs,0,0),1,rectangle_type_int,next_rank_z,1,newz_comm_world,req1s,ierr)
        call MPI_Irecv(var(0,0,0),1,rectangle_type_int,former_rank_z,1,newz_comm_world,req1r,ierr)

        call MPI_Isend(var(1,0,0),1,rectangle_type_int,former_rank_z,2,newz_comm_world,req2s,ierr)
        call MPI_Irecv(var(x_procs+1,0,0),1,rectangle_type_int,next_rank_z,2,newz_comm_world,req2r,ierr)

        call MPI_Wait(req1s,sta1s,ierr)
        call MPI_Wait(req1r,sta1r,ierr)
        call MPI_Wait(req2s,sta2s,ierr)
        call MPI_Wait(req2r,sta2r,ierr)
    end subroutine glue_int
...
use globals
    ...
    real(8),allocatable :: grad_u_procs(:,:,:,:,:), eps(:,:,:,:), minus(:,:,:,:)
    ...
    integer q, s
    integer,allocatable :: label(:,:,:)
    integer Vlabel(100), Vphi, Vlabel_sum, Vphi_sum
    integer labelmax
    ...
    call ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,eps,minus)
    call ini_op(taug_procs,nu_procs,phi_procs,label)
    ...
    ! if(comm_rank==0) then
    !     open(10,file="time.d",form='formatted')
    !     write(10,*) "start"
    !     close(10)
    ! endif
    klassnum = 1
    ...
    do zi = 1, z_procs
        do yi = 1, ymax+1
            do xi = 1, x_procs
                if(phi_procs(xi,yi,zi) >= (phi1+phi2)/2.0d0) then
                    tmp(:) = 0
                    tmp(1) = label(xi-1,yi,zi)
                    tmp(2) = label(xi,yi-1,zi)
                    tmp(3) = label(xi,yi,zi-1)
                    tmp(4) = label(xi,yi+1,zi-1)
                    tmp(5) = label(xi+1,yi,zi-1)
                    tmp(6) = label(xi,yi-1,zi-1)
                    tmp(7) = label(xi-1,yi,zi-1)
                    ...
                    if(tmp(1)+tmp(2)+tmp(3)+tmp(4)+tmp(5)+tmp(6)+tmp(7) == 0) then
                        label(xi,yi,zi) = klassnum
                        klass(klassnum) = klassnum
                        klassnum = klassnum + 1
                    else
                        ...
                        if(label(xi,yi+1,zi-1) > 0) then
                            ...
                            q = label(xi,yi+1,zi-1)
                        4   s = q
                            q = klass(q)
                            if(q /= s)  then 
                                go to 4
                            else
                                label(xi,yi+1,zi-1) = q
                            endif
                        endif
                        if(label(xi+1,yi,zi-1) > 0) then
                            klass(label(xi+1,yi,zi-1)) = label(xi,yi,zi)

                            q = label(xi+1,yi,zi-1)
                        5   s = q
                            q = klass(q)
                            if(q /= s)  then 
                                go to 5
                            else
                                label(xi+1,yi,zi-1) = q
                            endif
                        endif
                        if(label(xi,yi-1,zi-1) > 0) then
                            klass(label(xi,yi-1,zi-1)) = label(xi,yi,zi)
    ...
    do zi = 1, z_procs
        ...
        enddo
    enddo
    write(*,*) "parcolation is end"

    call glue_int(label)
    write(*,*) "glue_ini is end"
    ...
    do zi=1,z_procs
        do yi=1,ymax+1
            do xi=1,x_procs
                if(label(xi,yi,zi) > labelmax) then
                    labelmax = label(xi,yi,zi)
                endif
            enddo
        enddo
    enddo
    ...
    ! call MPI_Reduce(Vphi,Vphi_sum,1,MPI_INTEGER,MPI_SUM,0,new_comm_world,ierr)
    ! call MPI_Reduce(Vlabel,Vlabel_sum,1,MPI_INTEGER,MPI_SUM,0,new_comm_world,ierr)
    ! if(new_rank == 1) then
    !     open(43,file="v1.d",form='formatted')
    !     write(43,*) Vphi, Vlabel
    !     close(43)
    ! endif
    ! if(new_rank == 2) then
    !     open(43,file="v2.d",form='formatted')
    !     write(43,*) Vphi, Vlabel
    !     close(43)
    ! endif
    ! if(new_rank == 3) then
    !     open(43,file="v3.d",form='formatted')
    !     write(43,*) Vphi, Vlabel
    !     close(43)
    ! endif
    ! if(new_rank == 5) then
    !     open(43,file="v5.d",form='formatted')
    !     write(43,*) Vphi, Vlabel
    !     close(43)
    ! endif    
    ! if(new_rank == 6) then
    !     open(43,file="v6.d",form='formatted')
    !     write(43,*) Vphi, Vlabel
    !     close(43)
    ! endif    
    ! if(new_rank == 7) then
    !     open(43,file="v7.d",form='formatted')
    !     write(43,*) Vphi, Vlabel
    !     close(43)
    ! endif
    ! if(new_rank == 8) then
    !     open(43,file="v8.d",form='formatted')
    !     write(43,*) Vphi, Vlabel
    !     close(43)
    ! endif
