...
dfile4   = datadir + 'd70we20_ensemble.d'
...
def plot_y1y2(): # y1, y2プロット用
    ...
    ax1.plot(x4, y4, lw = 1, ls = 'none', marker = '*', ms = ms1*2, mew = lw1*0.5, mfc = 'none', color = 'black', 
            alpha = 0.3,     # 透明度
            clip_on = True, # プロット枠外にもプロットする
            zorder = 113,      # zorderが大きいほど前面に表示される
            label = r'$\mathrm{We}=20$') 
    ...
    ax1.plot(x4, y4, lw = lw1*1.0, ls = 'solid',  color = 'black', alpha = 0.3, clip_on = True, zorder = 23)
    ...
    # ### 保存
    fig.savefig(plotdir + "ensemble_kd_2" + ext, bbox_inches = "tight") # bbox_inches="tight"で余白をなくす
