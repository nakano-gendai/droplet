...
dfile4   = datadir + 'D70.d'
dfile5   = datadir + 'D100.d'
...
y2 = np.loadtxt(dfile2, usecols = 3, dtype = 'float64')
...
### サイズ、ラベルなどの設定
lx, ly = r'$\mathrm{We}$', r'$t_\mathrm{b}/\tau_{\mathrm{d}}$' # r''でTeX文字にできる
if fsize == 0:
    fs1, lw1, ms1 = 1., 1., 2.8
    tck_s1, alw = 3, 0.625
    ctxt1 = 'paper'
...
def plot_y1y2(): # y1, y2プロット用
    ...
    ax1.plot(x1, y1, lw = 1, ls = 'none', marker = 'o', ms = ms1*3.0, mew = lw1*1.2, mfc = 'none', color = 'black', 
            ...
            zorder = 2,
            label = r'$D_\mathrm{d}=30$') 
    ax1.plot(x2, y2, lw = 1, ls = 'none', marker = '^', ms = ms1*3.0, mew = lw1*1.2, mfc = 'none', color = 'black', 
            alpha = 0.8,     # 透明度
            clip_on = False, # プロット枠外にもプロットする
            zorder = 1,      # zorderが大きいほど前面に表示される
            label = r'$D_\mathrm{d}=40$') 
    ax1.plot(x3, y3, lw = 1, ls = 'none', marker = 'D', ms = ms1*2.5, mew = lw1*1.2, mfc = 'none', color = 'black', 
            alpha = 0.6,     # 透明度
            clip_on = False, # プロット枠外にもプロットする
            zorder = 1,      # zorderが大きいほど前面に表示される
            label = r'$D_\mathrm{d}=50$') 
    ax1.plot(x4, y4, lw = 1, ls = 'none', marker = '+', ms = ms1*2.5, mew = lw1*1.2, mfc = 'none', color = 'black', 
            alpha = 0.4,     # 透明度
            clip_on = False, # プロット枠外にもプロットする
            zorder = 100,      # zorderが大きいほど前面に表示される
            label = r'$D_\mathrm{d}=70$') 
    ax1.plot(x5, y5, lw = 1, ls = 'none', marker = '*', ms = ms1*3.0, mew = lw1*1.2, mfc = 'none', color = 'black', 
            alpha = 0.2,     # 透明度
            clip_on = False, # プロット枠外にもプロットする
            zorder = 100,      # zorderが大きいほど前面に表示される
            label = r'$D_\mathrm{d}=100$') 
    ...
    ax1.plot(x2, y2, lw = lw1*1.5, ls = 'solid',  color = 'black', alpha = 0.8, clip_on = True, zorder = 15)
    ax1.plot(x3, y3, lw = lw1*1.5, ls = 'solid',  color = 'black', alpha = 0.6, clip_on = True, zorder = 16)
    ax1.plot(x4, y4, lw = lw1*1.5, ls = 'solid',  color = 'black', alpha = 0.4, clip_on = True, zorder = 17)
    ax1.plot(x5, y5, lw = lw1*1.5, ls = 'solid',  color = 'black', alpha = 0.2, clip_on = True, zorder = 18)
    ...
    # ### 保存
    fig.savefig(plotdir + "We_taud" + ext, bbox_inches = "tight") # bbox_inches="tight"で余白をなくす
