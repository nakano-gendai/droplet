...
def plot_y1y2(): # y1, y2プロット用
    if fsize == 0:
        ...
        fig = plt.figure(figsize = (4.0, 2.5), dpi = 100, linewidth = 0)
    else:
    ...
    ### 水平線と鉛直線
    # ax1.axvline(7.0, lw = lw1*1.0, ls = 'dotted', alpha = 0.4, dashes = [1, 2], color = 'blue', label = r'line ($\Delta=16\Delta x$)')
    # ax1.axvline(15.0, lw = lw1*1.0, ls = 'dotted', alpha = 0.4, dashes = [1, 2], color = 'green', label = r'line ($\Delta=8\Delta x$)') # dashesで破線の間隔などを設定できる
    # ytext = -22.
    ...
    # ax1.axvline(7.0, lw = lw1*1.0, ls = 'dotted', alpha = 0.4, dashes = [1, 2], color = 'blue', label = r'filter line ($\Delta=16\Delta x$)')
    # ax1.axvline(15.0, lw = lw1*1.0, ls = 'dotted', alpha = 0.4, dashes = [1, 2], color = 'green', label = r'filter line ($\Delta=8\Delta x$)') # dashesで破線の間隔などを設定できる
    # ax1.axvline(0.392, lw = lw1*0.8, ls = 'dotted', dashes = [1, 2], color = 'red', label = r'$\pi / \Delta$') # dashesで破線の間隔などを設定できる
    # za=np.arange(1, 120, 0.0001)
    # ua=0.7*10.0**(-3.5)*za**(-5/3)
