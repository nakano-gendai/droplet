...
module globals
    ...
    !時間に関するパラメータ
    integer,parameter:: step_start = 1400000
    integer,parameter:: step = 100000000 !計算時間step
    !入力ディレクトリ
    character(*),parameter :: datadir_input = "/data/sht/nakanog/LES_turbulence_256_re80000_delta24/fg/"
    !出力ディレクトリ
    character(*),parameter :: datadir = "/data/sht/nakanog/LES_turbulence_256_re80000_delta24/"
    character(*),parameter :: datadir2 = "/data/sht/nakanog/LES_turbulence_256_re80000_delta24/"

    !無次元数
    real(8),parameter:: Re = 80000.0d0 !粒子レイノルズ数

    !LESパラメータ
    real(8),parameter:: delta = 24.0d0*ds
...
contains
    ...
    !===============================================================================================================
        ...
        filename=datadir_input//trim(adjustl(filename))//'_'//trim(adjustl(filename3))//'_'//trim(adjustl(filename2))//'_fg.bin' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
