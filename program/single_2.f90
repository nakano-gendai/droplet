...
module globals
    ...
    !出力ディレクトリ
    character(*),parameter :: datadir = "/data/sht/nakanog/LES13/"
    character(*),parameter :: datadir2 = "/data/sht/nakanog/LES13/fg/"
...
contains
    ...
    subroutine ini(g_procs,gnext_procs,geq_procs,p_procs,u1_procs,u2_procs,u3_procs,tensor,x_m,y_m,eps,eps2)
...
!================================乱数の発生（標準正規分布）=====================================
    ...
    subroutine ini_op(kinetic_procs,forcex,forcey,forcez,strain_procs,tensor,nue,taug_procs,g_procs,geq_procs,Q_procs,S_procs)
        ...
        real(8),allocatable :: Q_procs(:,:,:), S_procs(:,:,:)
        ...
        allocate(S_procs(1:x_procs,1:ymax+1,1:z_procs))
        ...
        do zi = 1, z_procs
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    Q_procs(xi,yi,zi) = 0.0d0
                    do beta = 1, 3
                        do alpha = 1, 3
                            Q_procs(xi,yi,zi) = Q_procs(xi,yi,zi) + tensor(alpha,beta,xi,yi,zi)**2.0d0
                        enddo
                    enddo
                    S_procs(xi,yi,zi) = (nu**2.0d0 + 18.0d0*Cs*Cs*delta*delta*Q_procs(xi,yi,zi)**0.5d0)**0.5d0 - nu
                    S_procs(xi,yi,zi) = S_procs(xi,yi,zi) / (6.0d0*Cs*Cs*delta*delta)
                    nue(xi,yi,zi) = Cs * Cs * delta * delta * S_procs(xi,yi,zi)
    ...
    subroutine physics(p_procs,u1_procs,u2_procs,u3_procs,g_procs,gnext_procs,geq_procs,nue,taug_procs,tensor,Q_procs,S_procs)
        ...
        real(8),intent(inout) :: S_procs(1:x_procs,1:ymax+1,1:z_procs)
...
use globals
    ...
    !LESの計算に使用する変数
    real(8),allocatable :: tensor(:,:,:,:,:), strain_procs(:,:,:,:,:)
    real(8),allocatable :: nue(:,:,:)
    real(8),allocatable :: taug_procs(:,:,:)
    real(8),allocatable :: Q_procs(:,:,:), S_procs(:,:,:)
...
!================================初期条件・配列のallocate======================================
    ...
    call ini(g_procs,gnext_procs,geq_procs,p_procs,u1_procs,u2_procs,u3_procs,tensor,x_m,y_m,eps,eps2)
    call ini_op(kinetic_procs,forcex,forcey,forcez,strain_procs,tensor,nue,taug_procs,g_procs,geq_procs,Q_procs,S_procs)
...
!==========================物理量の計算(OMP)=====================================================
    call physics(p_procs,u1_procs,u2_procs,u3_procs,g_procs,gnext_procs,geq_procs,nue,taug_procs,tensor,Q_procs,S_procs)
!================================出力==================================
