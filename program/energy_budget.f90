...
!===================================初期条件の設定================================================
    ...
    subroutine ini_energy(left1_procs,left2_procs,right1_procs,right2_procs,right3_procs,right4_procs,right5_procs)
        real(8),allocatable :: left1_procs(:,:,:), left2_procs(:,:,:)
        real(8),allocatable :: right1_procs(:,:,:), right2_procs(:,:,:), right3_procs(:,:,:)
        real(8),allocatable :: right4_procs(:,:,:), right5_procs(:,:,:)

        !以下はのりしろ無しの変数
        allocate(left1_procs(1:x_procs,1:y_procs,1:zmax+1))
        allocate(left2_procs(1:x_procs,1:y_procs,1:zmax+1))
        allocate(right1_procs(1:x_procs,1:y_procs,1:zmax+1))
        allocate(right2_procs(1:x_procs,1:y_procs,1:zmax+1))
        allocate(right3_procs(1:x_procs,1:y_procs,1:zmax+1))
        allocate(right4_procs(1:x_procs,1:y_procs,1:zmax+1))
        allocate(right5_procs(1:x_procs,1:y_procs,1:zmax+1))
    ...
    subroutine ini_energy2(kinetic_procs,grad_kinetic_procs,A_procs,strain_procs,u_strain_procs,korteg_procs,grad_A_procs,kinetic_former_procs)
        ...
        allocate(grad_kinetic_procs(1:3,1:x_procs,1:y_procs,1:zmax+1))
        allocate(A_procs(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(strain_procs(1:3,1:3,1:x_procs,1:y_procs,1:zmax+1))
        allocate(u_strain_procs(1:3,1:3,1:x_procs,1:y_procs,1:zmax+1))
    ...
    subroutine ini_energy4(s_tensor11,s_tensor12,s_tensor13,s_tensor21,s_tensor22,s_tensor23,s_tensor31,s_tensor32,s_tensor33)
        real(8),allocatable :: s_tensor11(:,:,:), s_tensor12(:,:,:), s_tensor13(:,:,:)
        real(8),allocatable :: s_tensor21(:,:,:), s_tensor22(:,:,:), s_tensor23(:,:,:)
        real(8),allocatable :: s_tensor31(:,:,:), s_tensor32(:,:,:), s_tensor33(:,:,:)

        !以下はのりしろ無しの変数
        allocate(s_tensor11(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor12(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor13(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor21(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor22(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor23(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor31(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor32(0:x_procs+1,0:y_procs+1,0:zmax+2))
        allocate(s_tensor33(0:x_procs+1,0:y_procs+1,0:zmax+2))
...
!============================エネルギー計算========================
    ...
    do zi = 1, zmax+1
        do yi = 1, y_procs
            do xi = 1, x_procs
                left2_procs(xi,yi,zi) = u1_procs(xi,yi,zi)*grad_kinetic_procs(1,xi,yi,zi) &
                                        + u2_procs(xi,yi,zi)*grad_kinetic_procs(2,xi,yi,zi) &
                                        + u3_procs(xi,yi,zi)*grad_kinetic_procs(3,xi,yi,zi)
            enddo
        enddo
    enddo
    ...
    do zi=1,zmax+1
        do yi=1,y_procs
            do xi=1,x_procs
                do alpha=1,3
                    grad_A_procs(alpha,xi,yi,zi) = 0.0d0
                    do i = 2,15
                        grad_A_procs(alpha,xi,yi,zi) = grad_A_procs(alpha,xi,yi,zi) &
                                                        + cr(alpha,i)*A_procs(xi+cx(i),yi+cy(i),zi+cz(i))
                    enddo
                    grad_A_procs(alpha,xi,yi,zi) = grad_A_procs(alpha,xi,yi,zi)/(10.0d0*ds)
                enddo
            enddo
        enddo
    enddo
    do zi = 1, zmax+1
        do yi = 1, y_procs
            do xi = 1, x_procs
                right1_procs(xi,yi,zi) = u1_procs(xi,yi,zi)*grad_A_procs(1,xi,yi,zi) &
                                        + u2_procs(xi,yi,zi)*grad_A_procs(2,xi,yi,zi) &
                                        + u3_procs(xi,yi,zi)*grad_A_procs(3,xi,yi,zi)
                
                right1_procs(xi,yi,zi) = -1.0d0 * right1_procs(xi,yi,zi)
            enddo
        enddo
    enddo
    ...
    do zi = 1, zmax+1
        do yi = 1, y_procs
            do xi = 1, x_procs
                u_strain_procs(1,1,xi,yi,zi) = u1_procs(xi,yi,zi)*strain_procs(1,1,xi,yi,zi)
                u_strain_procs(2,1,xi,yi,zi) = u2_procs(xi,yi,zi)*strain_procs(2,1,xi,yi,zi)
                u_strain_procs(3,1,xi,yi,zi) = u3_procs(xi,yi,zi)*strain_procs(3,1,xi,yi,zi)

                u_strain_procs(1,2,xi,yi,zi) = u1_procs(xi,yi,zi)*strain_procs(1,2,xi,yi,zi)
                u_strain_procs(2,2,xi,yi,zi) = u2_procs(xi,yi,zi)*strain_procs(2,2,xi,yi,zi)
                u_strain_procs(3,2,xi,yi,zi) = u3_procs(xi,yi,zi)*strain_procs(3,2,xi,yi,zi)

                u_strain_procs(1,3,xi,yi,zi) = u1_procs(xi,yi,zi)*strain_procs(1,3,xi,yi,zi)
                u_strain_procs(2,3,xi,yi,zi) = u2_procs(xi,yi,zi)*strain_procs(2,3,xi,yi,zi)
                u_strain_procs(3,3,xi,yi,zi) = u3_procs(xi,yi,zi)*strain_procs(3,3,xi,yi,zi)
            enddo
        enddo
    enddo
    do zi = 1, zmax+1
        do yi = 1, y_procs
            do xi = 1, x_procs
                s_tensor11(xi,yi,zi) = u_strain_procs(1,1,xi,yi,zi)
                s_tensor21(xi,yi,zi) = u_strain_procs(2,1,xi,yi,zi)
                s_tensor31(xi,yi,zi) = u_strain_procs(3,1,xi,yi,zi)

                s_tensor12(xi,yi,zi) = u_strain_procs(1,2,xi,yi,zi)
                s_tensor22(xi,yi,zi) = u_strain_procs(2,2,xi,yi,zi)
                s_tensor32(xi,yi,zi) = u_strain_procs(3,2,xi,yi,zi)

                s_tensor13(xi,yi,zi) = u_strain_procs(1,3,xi,yi,zi)
                s_tensor23(xi,yi,zi) = u_strain_procs(2,3,xi,yi,zi)
                s_tensor33(xi,yi,zi) = u_strain_procs(3,3,xi,yi,zi)
            enddo
        enddo
    enddo
    call glue(s_tensor11)
    call glue(s_tensor21)
    call glue(s_tensor31)

    call glue(s_tensor12)
    call glue(s_tensor22)
    call glue(s_tensor32)

    call glue(s_tensor13)
    call glue(s_tensor23)
    call glue(s_tensor33)
            ...
            do xi=1,x_procs
                ...
                do i = 2,15
                    grad_tensor11(xi,yi,zi) = grad_tensor11(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor11(xi+cx(i),yi+cy(i),zi+cz(i))
                    grad_tensor21(xi,yi,zi) = grad_tensor21(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor21(xi+cx(i),yi+cy(i),zi+cz(i))
                    grad_tensor31(xi,yi,zi) = grad_tensor31(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor31(xi+cx(i),yi+cy(i),zi+cz(i))
                enddo
            ...
            do xi=1,x_procs
                ...
                do i = 2,15
                    grad_tensor12(xi,yi,zi) = grad_tensor12(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor12(xi+cx(i),yi+cy(i),zi+cz(i))
                    grad_tensor22(xi,yi,zi) = grad_tensor22(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor22(xi+cx(i),yi+cy(i),zi+cz(i))
                    grad_tensor32(xi,yi,zi) = grad_tensor32(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor32(xi+cx(i),yi+cy(i),zi+cz(i))
                enddo
            ...
            do xi=1,x_procs
                ...
                do i = 2,15
                    grad_tensor13(xi,yi,zi) = grad_tensor13(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor13(xi+cx(i),yi+cy(i),zi+cz(i))
                    grad_tensor23(xi,yi,zi) = grad_tensor23(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor23(xi+cx(i),yi+cy(i),zi+cz(i))
                    grad_tensor33(xi,yi,zi) = grad_tensor33(xi,yi,zi) &
                                            + cr(alpha,i)*s_tensor33(xi+cx(i),yi+cy(i),zi+cz(i))
                enddo
    ...
    do zi=1,zmax+1
        do yi=1,y_procs
            do xi=1,x_procs
                right2_procs(xi,yi,zi) = grad_tensor11(xi,yi,zi) + grad_tensor21(xi,yi,zi) + grad_tensor31(xi,yi,zi) &
                                        + grad_tensor12(xi,yi,zi) + grad_tensor22(xi,yi,zi) + grad_tensor32(xi,yi,zi) &
                                        + grad_tensor13(xi,yi,zi) + grad_tensor23(xi,yi,zi) + grad_tensor33(xi,yi,zi)
                
                right2_procs(xi,yi,zi) = right2_procs(xi,yi,zi) * 2.0d0 * nu_procs(xi,yi,zi)
            enddo
        enddo
    enddo
    ...
    do zi = 1, zmax + 1
        do yi = 1, y_procs
            do xi = 1, x_procs
                ...
                right3_procs(xi,yi,zi) = right3_procs(xi,yi,zi) * (-2.0d0) * nu_procs(xi,yi,zi)
            enddo
        enddo
    enddo
    ...
    do zi=1,zmax+1
        do yi=1,y_procs
            do xi=1,x_procs
                right4_procs(xi,yi,zi) = u1_procs(xi,yi,zi)*grad_tensor11(xi,yi,zi) + u2_procs(xi,yi,zi)*grad_tensor21(xi,yi,zi) + u3_procs(xi,yi,zi)*grad_tensor31(xi,yi,zi) &
    ...
    do zi=1,zmax+1
        do yi=1,y_procs
            do xi=1,x_procs
                right5_procs(xi,yi,zi) = u1_procs(xi,yi,zi)*forcex(xi,yi,zi) + u2_procs(xi,yi,zi)*forcey(xi,yi,zi) + u3_procs(xi,yi,zi)*forcez(xi,yi,zi)
            enddo
        enddo
    enddo
    ...
    do zi=1,zmax+1
        do yi=1,y_procs
            do xi=1,x_procs
                ...
                right3_sum = right3_sum + right3_procs(xi,yi,zi)
                right4_sum = right4_sum + right4_procs(xi,yi,zi)
                right5_sum = right5_sum + right5_procs(xi,yi,zi)
            enddo
        enddo
    enddo
    ...
    call MPI_Reduce(right3_sum, right3_all, 1, MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
    call MPI_Reduce(right4_sum, right4_all, 1, MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
    call MPI_Reduce(right5_sum, right5_all, 1, MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
