...
module globals
    ...
    character(*),parameter :: datadir = "/data/sht/nakanog/LES_test/"
    character(*),parameter :: datadir2 = "/data/sht/nakanog/LES_test/fg/"

    !LESパラメータ
    real(8),parameter:: delta = 2.0d0*ds
    real(8),parameter:: Cs = 0.173d0

    !無次元数
    real(8),parameter:: We = 3.0d0 !粒子ウエーバー数
    real(8),parameter:: Re = 8000.0d0 !粒子レイノルズ数
    ...
    real(8),parameter:: umax = 0.01d0 !最大流速
    ...
    real(8),parameter:: Anu = 0.1d0 !粘性係数が小さいときに値を入れると良い
    ...
    integer rectangle_type, rectangle_type_f, rectangle_type_s, xtype, xtype_f, xtype_s
...
contains
    ...
    subroutine ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,x_m,y_m,eps,eps2)
    ...
    !========================並列数・コミュニケータ分割・通信先設定========================
        ...
        call MPI_Type_Vector(ymax+3,3*3*x_procs,3*3*(x_procs+2),MPI_REAL8,xtype_s,ierr)
        call MPI_Type_Commit(xtype_s,ierr)
        ...
        call MPI_Type_Vector((z_procs+2)*(ymax+3),3*3,3*3*(x_procs+2),MPI_REAL8,rectangle_type_s,ierr)
        call MPI_Type_Commit(rectangle_type_s,ierr)
    !===============================================================================================================
        !以下はのりしろ有りの変数
...
!================================乱数の発生（標準正規分布）=====================================
        ...
        do zi=1,z_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    u1_procs(xi,yi,zi) = umax*sin(kw*dble(xi))*cos(kw*dble(zi))
                    u2_procs(xi,yi,zi) = 0.0d0
                    u3_procs(xi,yi,zi) = -umax*cos(kw*dble(xi))*sin(kw*dble(zi))
                    p_procs(xi,yi,zi) = 1.0d0/3.0d0 + 1.0d0/4.0d0*umax*umax*(cos(2.0d0*kw*dble(xi)) + cos(2.0d0*kw*dble(zi)))
                    phi_procs(xi,yi,zi) = phi1

                    u1_procs(xi,yi,zi) = u1_procs(xi,yi,zi) + x_m(xi,yi,zi) * ran
                    u2_procs(xi,yi,zi) = u2_procs(xi,yi,zi) + y_m(xi,yi,zi) * ran
                    u3_procs(xi,yi,zi) = u3_procs(xi,yi,zi) + 0.5d0 * ( x_m(xi,yi,zi) + y_m(xi,yi,zi) ) * ran
                enddo
            enddo
        enddo
        !$omp end do
    ...
    subroutine ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs,forcex,forcey,forcez)
        real(8),intent(in):: phi_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),allocatable :: taug_procs(:,:,:), nu_procs(:,:,:), kinetic_procs(:,:,:)
    ...
    subroutine ini_les(nue,strain,grad_u_procs,grad_strain,force_nux,force_nuy,force_nuz,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
        real(8),intent(in) :: grad_u_procs(1:3,1:3,1:x_procs,1:ymax+1,1:z_procs)
        real(8),allocatable :: nue(:,:,:), strain(:,:,:,:,:),force_nux(:,:,:),force_nuy(:,:,:),force_nuz(:,:,:)
        real(8),allocatable :: grad_strain(:,:,:,:,:)
        real(8),allocatable :: u1_procs_fliter(:,:,:),u2_procs_fliter(:,:,:),u3_procs_fliter(:,:,:),p_procs_fliter(:,:,:)

        allocate(nue(1:x_procs,1:ymax+1,1:z_procs))
        allocate(strain(1:3,1:3,0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(grad_strain(1:3,1:3,1:x_procs,1:ymax+1,1:z_procs))
        allocate(force_nux(0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(force_nuy(0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(force_nuz(0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(u1_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(u2_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(u3_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1))
        allocate(p_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1))
        ...
        force_nuy(:,:,:) = 0.0d0
        force_nuz(:,:,:) = 0.0d0
        u1_procs_fliter(:,:,:) = 0.0d0
        u2_procs_fliter(:,:,:) = 0.0d0
        u3_procs_fliter(:,:,:) = 0.0d0
        p_procs_fliter(:,:,:) = 0.0d0


        call strain_cal(grad_u_procs,strain)
        call nue_cal(nue,grad_u_procs)
    end subroutine ini_les
    ...
    subroutine glue_s(var_s)
        real(8),intent(inout) :: var_s(1:3,1:3,0:x_procs+1,0:ymax+2,0:z_procs+1)
        integer alpha, beta, xi, zi

        do zi=1,z_procs
            do xi=1,x_procs
                do beta = 1,3
                    do alpha = 1,3
                        var_s(alpha,beta,xi,0,zi) = var_s(alpha,beta,xi,ymax+1,zi)
                        var_s(alpha,beta,xi,ymax+2,zi) = var_s(alpha,beta,xi,1,zi)
                    enddo
                enddo
            enddo
        enddo
        ...
        !X世界でののりしろ通信
        call MPI_Isend(var_s(1,1,1,0,z_procs),1,xtype_s,next_rank_x,1,newx_comm_world,req1s,ierr)
        call MPI_Irecv(var_s(1,1,1,0,0),1,xtype_s,former_rank_x,1,newx_comm_world,req1r,ierr)

        call MPI_Isend(var_s(1,1,1,0,1),1,xtype_s,former_rank_x,2,newx_comm_world,req2s,ierr)
        call MPI_Irecv(var_s(1,1,1,0,z_procs+1),1,xtype_s,next_rank_x,2,newx_comm_world,req2r,ierr)
        ...
        !Z世界でののりしろ通信
        call MPI_Isend(var_s(1,1,x_procs,0,0),1,rectangle_type_s,next_rank_z,1,newz_comm_world,req1s,ierr)
        call MPI_Irecv(var_s(1,1,0,0,0),1,rectangle_type_s,former_rank_z,1,newz_comm_world,req1r,ierr)

        call MPI_Isend(var_s(1,1,1,0,0),1,rectangle_type_s,former_rank_z,2,newz_comm_world,req2s,ierr)
        call MPI_Irecv(var_s(1,1,x_procs+1,0,0),1,rectangle_type_s,next_rank_z,2,newz_comm_world,req2r,ierr)

        call MPI_Wait(req1s,sta1s,ierr)
        call MPI_Wait(req1r,sta1r,ierr)
        call MPI_Wait(req2s,sta2s,ierr)
        call MPI_Wait(req2r,sta2r,ierr)
    end subroutine glue_s

    subroutine MPI_boundary(phi_procs,u1_procs,u2_procs,u3_procs,p_procs,f_procs,g_procs,taug_procs)
    ...
    subroutine filter_x(u1_procs,u2_procs,u3_procs,p_procs,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
        real(8),intent(inout) :: u1_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u2_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u3_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: p_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u1_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u2_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u3_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: p_procs_fliter(0:x_procs+1,0:ymax+2,0:z_procs+1)
        integer xi, yi, zi

        do zi = 1, z_procs
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    u1_procs_fliter(xi,yi,zi) = u1_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( u1_procs(xi-1,yi,zi) &
                                                - 2.0d0 * u1_procs(xi,yi,zi) &
                                                + u1_procs(xi+1,yi,zi) ) / ds**2.0d0
                    u2_procs_fliter(xi,yi,zi) = u2_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( u2_procs(xi-1,yi,zi) &
                                                - 2.0d0 * u2_procs(xi,yi,zi) &
                                                + u2_procs(xi+1,yi,zi) ) / ds**2.0d0
                    u3_procs_fliter(xi,yi,zi) = u3_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( u3_procs(xi-1,yi,zi) &
                                                - 2.0d0 * u3_procs(xi,yi,zi) &
                                                + u3_procs(xi+1,yi,zi) ) / ds**2.0d0
                    p_procs_fliter(xi,yi,zi) = p_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( p_procs(xi-1,yi,zi) &
                                                - 2.0d0 * p_procs(xi,yi,zi) &
                                                + p_procs(xi+1,yi,zi) ) / ds**2.0d0
                enddo
            enddo
        enddo
        ...
        do zi = 1, z_procs
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    u1_procs(xi,yi,zi) = u1_procs_fliter(xi,yi,zi)
                    u2_procs(xi,yi,zi) = u2_procs_fliter(xi,yi,zi)
                    u3_procs(xi,yi,zi) = u3_procs_fliter(xi,yi,zi)
                    p_procs(xi,yi,zi) = p_procs_fliter(xi,yi,zi)
                enddo
            enddo
        enddo
    end subroutine filter_x

    subroutine filter_y(u1_procs,u2_procs,u3_procs,p_procs,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
        real(8),intent(inout) :: u1_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u2_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u3_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: p_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
            ...
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    u1_procs_fliter(xi,yi,zi) = u1_procs(xi,yi,zi) &
                                                ...
                                                * ( u1_procs(xi,yi-1,zi) &
                                                - 2.0d0 * u1_procs(xi,yi,zi) &
                                                + u1_procs(xi,yi+1,zi) ) / ds**2.0d0
                    u2_procs_fliter(xi,yi,zi) = u2_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( u2_procs(xi,yi-1,zi) &
                                                - 2.0d0 * u2_procs(xi,yi,zi) &
                                                + u2_procs(xi,yi+1,zi) ) / ds**2.0d0
                    u3_procs_fliter(xi,yi,zi) = u3_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( u3_procs(xi,yi-1,zi) &
                                                - 2.0d0 * u3_procs(xi,yi,zi) &
                                                + u3_procs(xi,yi+1,zi) ) / ds**2.0d0
                    p_procs_fliter(xi,yi,zi) = p_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( p_procs(xi,yi-1,zi) &
                                                - 2.0d0 * p_procs(xi,yi,zi) &
                                                + p_procs(xi,yi+1,zi) ) / ds**2.0d0
                enddo
            enddo
        enddo
    ...
    subroutine filter_z(u1_procs,u2_procs,u3_procs,p_procs,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
        real(8),intent(inout) :: u1_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u2_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: u3_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(inout) :: p_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
            ...
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    u1_procs_fliter(xi,yi,zi) = u1_procs(xi,yi,zi) &
                                                ...
                                                * ( u1_procs(xi,yi,zi-1) &
                                                - 2.0d0 * u1_procs(xi,yi,zi) &
                                                + u1_procs(xi,yi,zi+1) ) / ds**2.0d0
                    u2_procs_fliter(xi,yi,zi) = u2_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( u2_procs(xi,yi,zi-1) &
                                                - 2.0d0 * u2_procs(xi,yi,zi) &
                                                + u2_procs(xi,yi,zi+1) ) / ds**2.0d0
                    u3_procs_fliter(xi,yi,zi) = u3_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( u3_procs(xi,yi,zi-1) &
                                                - 2.0d0 * u3_procs(xi,yi,zi) &
                                                + u3_procs(xi,yi,zi+1) ) / ds**2.0d0
                    p_procs_fliter(xi,yi,zi) = p_procs(xi,yi,zi) &
                                                + delta * delta / 24.0d0 &
                                                * ( p_procs(xi,yi,zi-1) &
                                                - 2.0d0 * p_procs(xi,yi,zi) &
                                                + p_procs(xi,yi,zi+1) ) / ds**2.0d0
                enddo
            enddo
        enddo
    ...
    subroutine strain_cal(grad_u_procs,strain)
        real(8),intent(in):: grad_u_procs(1:3,1:3,1:x_procs,1:ymax+1,1:z_procs)
        real(8),intent(out):: strain(1:3,1:3,0:x_procs+1,0:ymax+2,0:z_procs+1)
        ...
        do zi = 1, z_procs
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    do beta = 1, 3
                        do alpha = 1, 3
                            strain(alpha,beta,xi,yi,zi) = 0.5d0*( grad_u_procs(alpha,beta,xi,yi,zi) + grad_u_procs(beta,alpha,xi,yi,zi) )
                        enddo
                    enddo
                enddo
            enddo
    ...
    subroutine nue_cal(nue,strain)
        real(8),intent(in) :: strain(1:3,1:3,1:x_procs,1:ymax+1,1:z_procs)
        ...
        do zi = 1, z_procs
            do yi = 1, ymax+1
                do xi = 1, xmax
                    ...
                    do beta = 1, 3
                        do alpha = 1, 3
                            nue(xi,yi,zi) = nue(xi,yi,zi) + 2.0d0*(strain(alpha,beta,xi,yi,zi)*strain(alpha,beta,xi,yi,zi))
                        enddo
                    enddo
                    nue(xi,yi,zi) = (Cs*delta)**2.0d0 * nue(xi,yi,zi)**0.5d0
                enddo
            enddo
        enddo
    end subroutine nue_cal

    subroutine force_nu_cal(force_nux,force_nuy,force_nuz,strain,grad_strain,nue)
        real(8),intent(out):: force_nux(0:x_procs+1,0:ymax+2,0:z_procs+1), force_nuy(0:x_procs+1,0:ymax+2,0:z_procs+1), force_nuz(0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(out) :: grad_strain(1:3,1:3,1:x_procs,1:ymax+1,1:z_procs)
        real(8),intent(in):: strain(1:3,1:3,0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(in) :: nue(0:x_procs+1,0:ymax+2,0:z_procs+1)

        integer xi, yi, zi, alpha, i
        
        do zi=1,z_procs
            do yi=1,ymax+1
                do xi=1,x_procs
                    do alpha=1,3
                        grad_strain(1,alpha,xi,yi,zi) = 0.0d0
                        grad_strain(2,alpha,xi,yi,zi) = 0.0d0
                        grad_strain(3,alpha,xi,yi,zi) = 0.0d0
                        do i = 2,15
                            grad_strain(1,alpha,xi,yi,zi) = grad_strain(1,alpha,xi,yi,zi) &
                                                            + cr(alpha,i)*strain(1,alpha,xi+cx(i),yi+cy(i),zi+cz(i))
                            grad_strain(2,alpha,xi,yi,zi) = grad_strain(2,alpha,xi,yi,zi) &
                                                            + cr(alpha,i)*strain(2,alpha,xi+cx(i),yi+cy(i),zi+cz(i))
                            grad_strain(3,alpha,xi,yi,zi) = grad_strain(3,alpha,xi,yi,zi) &
                                                            + cr(alpha,i)*strain(3,alpha,xi+cx(i),yi+cy(i),zi+cz(i))
                        enddo
                        grad_strain(1,alpha,xi,yi,zi) = grad_strain(1,alpha,xi,yi,zi)/(10.0d0*ds)
                        grad_strain(2,alpha,xi,yi,zi) = grad_strain(2,alpha,xi,yi,zi)/(10.0d0*ds)
                        grad_strain(3,alpha,xi,yi,zi) = grad_strain(3,alpha,xi,yi,zi)/(10.0d0*ds)
                    enddo
                    force_nux(xi,yi,zi) = 2.0d0 * nue(xi,yi,zi) * (grad_strain(1,1,xi,yi,zi) + grad_strain(1,2,xi,yi,zi) + grad_strain(1,3,xi,yi,zi))
                    force_nuy(xi,yi,zi) = 2.0d0 * nue(xi,yi,zi) * (grad_strain(2,1,xi,yi,zi) + grad_strain(2,2,xi,yi,zi) + grad_strain(2,3,xi,yi,zi))
                    force_nuz(xi,yi,zi) = 2.0d0 * nue(xi,yi,zi) * (grad_strain(3,1,xi,yi,zi) + grad_strain(3,2,xi,yi,zi) + grad_strain(3,3,xi,yi,zi))
                enddo
            enddo
        enddo
    ...
    subroutine fg_cal(fnext_procs,f_procs,feq_procs,gnext_procs,g_procs,geq_procs,taug_procs,forcex,forcey,forcez,force_nux,force_nuy,force_nuz)
        ...
        !渦粘性外力
        real(8),intent(in):: force_nux(0:x_procs+1,0:ymax+2,0:z_procs+1), force_nuy(0:x_procs+1,0:ymax+2,0:z_procs+1), force_nuz(0:x_procs+1,0:ymax+2,0:z_procs+1)
        integer i, xi, yi, zi
                    ...
                    do i=1,15
                        ...
                        gnext_procs(i,xi,yi,zi) = g_procs(i,xi-cx(i),yi-cy(i),zi-cz(i)) &
                                                ...
                                                * (dble(cx(i))*forcex(xi-cx(i),yi-cy(i),zi-cz(i)) &
                                                ...
                                                + 3.0d0 * ds * E(i) &
                                                * (dble(cx(i))*force_nux(xi-cx(i),yi-cy(i),zi-cz(i)) &
                                                + dble(cy(i))*force_nuy(xi-cx(i),yi-cy(i),zi-cz(i)) &
                                                + dble(cz(i))*force_nuz(xi-cx(i),yi-cy(i),zi-cz(i)))
                    enddo
                enddo
            enddo
        enddo
...
use globals
    ...
    real(8),allocatable :: nue(:,:,:), strain(:,:,:,:,:), force_nux(:,:,:), force_nuy(:,:,:), force_nuz(:,:,:)
    real(8),allocatable :: grad_strain(:,:,:,:,:)
    real(8),allocatable :: p_procs_fliter(:,:,:)
    real(8),allocatable :: u1_procs_fliter(:,:,:), u2_procs_fliter(:,:,:), u3_procs_fliter(:,:,:)
    !その他変数
    real(8) U(1:3) !壁の移動速度
    integer n, xi, yi, zi
...
!================================初期条件・配列のallocate======================================
    ...
    call ini(g_procs,gnext_procs,f_procs,fnext_procs,feq_procs,geq_procs,phi_procs,p_procs,u1_procs,u2_procs,u3_procs,grad_phi_procs,gphi_procs,lap_phi_procs,p0_procs,grad_u_procs,x_m,y_m,eps,eps2)
    call ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs,forcex,forcey,forcez)
    call ini_les(nue,strain,grad_u_procs,grad_strain,force_nux,force_nuy,force_nuz,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
...
DO n=1,step
!========================のりしろ境界=======================================================
    call MPI_boundary(phi_procs,u1_procs,u2_procs,u3_procs,p_procs,f_procs,g_procs,taug_procs)
!========================フィルター========================================================
    call filter_x(u1_procs,u2_procs,u3_procs,p_procs,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
    call filter_y(u1_procs,u2_procs,u3_procs,p_procs,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
    call filter_z(u1_procs,u2_procs,u3_procs,p_procs,u1_procs_fliter,u2_procs_fliter,u3_procs_fliter,p_procs_fliter)
!=======================諸変数の計算(OMP)======================================================
    call various_cal(lap_phi_procs,grad_phi_procs,grad_u_procs,p0_procs,phi_procs,u1_procs,u2_procs,u3_procs)
    call gphi_cal(gphi_procs,grad_phi_procs)
!=======================局所平衡分布関数の計算(OMP)==============================================
...
!=======================LES変数の計算（MPIのみ）=============================================
    call strain_cal(grad_u_procs,strain)
    call nue_cal(nue,strain)
    call force_nu_cal(force_nux,force_nuy,force_nuz,strain,grad_strain,nue)
!=======================平衡分布関数fgの計算(OMP)=================================================
    call fg_cal(fnext_procs,f_procs,feq_procs,gnext_procs,g_procs,geq_procs,taug_procs,forcex,forcey,forcez,force_nux,force_nuy,force_nuz)
!==========================物理量の計算(OMP)=====================================================
...
!================================出力==================================
    ...
    if(mod(n,4000)==0) then
        ...
        if(new_rank == 0) then
            ...
            if(n == 4000) then
                open(11,file="./kinetic_les_test.d")
                write(11,*) dble(n), dble(n)*umax/D_vortex, kinetic_ave
                close(11)
            else
                open(11,file="./kinetic_les_test.d",action="write",position="append")
                write(11,*) dble(n), dble(n)*umax/D_vortex, kinetic_ave
                close(11)
            endif
        endif
