...
use globals
    ...
    integer set
    !ティレクトリー読み込み
    character(*),parameter :: datadir = "/data/sht/nakanog/taylor_re12000_drop4/"
    !ディレクトリ作成
    character(*),parameter :: datadir2 = "/data/sht/nakanog/taylor_re12000_drop4/gnu/"
    ...
    ! step = 100000
        ...
        filename=datadir2//"0_"//trim(adjustl(filename))//'.d' !adjustlで左寄せにしてからtrimで末尾の空白除去，拡張子等をくっつける
        print *, filename !表示してみる
        ! open(11, file=filename, form="unformatted", status='replace') 
        open(11, file=filename, status='replace') 
        do zi=0,zmax
            do xi=0,xmax
                write(11,*) dble(xi), dble(zi),  phi(xi,zi)
            enddo
            write(11,*)
        enddo
        close(11)
