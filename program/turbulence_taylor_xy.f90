...
module globals
    ...
    !出力ディレクトリ
    character(*),parameter :: datadir_output = "/data/lng/nakanog/taylor_512_re100000_large_xy/large/"
...
contains
    ...
    subroutine taylor_re_cal(u_rms_ave,u_rms_ave_sum,u_grad_ave,u_grad_ave_sum,taylor_re,taylor_length,u_rms,u_grad)
        ...
        if(comm_rank == 0) then
            ...
            if(n == step_start) then
                open(103,file="./taylor_para_xy.d")
                write(103,*) dble(n), dble(n)*umax/D_vortex, taylor_length, taylor_re, u_rms_ave_sum
                close(103)
            else
                open(103,file="./taylor_para_xy.d",action="write",position="append")
                write(103,*) dble(n), dble(n)*umax/D_vortex, taylor_length, taylor_re, u_rms_ave_sum
                close(103)
            endif
        endif
    ...
    subroutine energy_dissipation_cal(strain_procs,energy_dissipation,energy_dissipation_sum,Kolmogorov_scale)
        ...
        if(comm_rank == 0) then
            ...
            if(n == step_start) then
                open(102,file="./kolmogorov_xy.d")
                write(102,*) dble(n), dble(n)*umax/D_vortex, Kolmogorov_scale, energy_dissipation, nu1
                close(102)
            else
                open(102,file="./kolmogorov_xy.d",action="write",position="append")
                write(102,*) dble(n), dble(n)*umax/D_vortex, Kolmogorov_scale, energy_dissipation, nu1
                close(102)
            endif
        endif
