...
module globals
    ...
    integer,parameter:: case_initial_num = 16 !最初のケース番号
    integer,parameter:: case_end_num = case_initial_num !最後のケース番号

    !読み込みディレクトリ
    character(*),parameter :: datadir_input = "/data/sht/nakanog/IHT_drop_d70_we10/"
    character(*),parameter :: datadir_input2 = "/data/sht/nakanog/IHT_drop_d70_we10/u/"
    !出力ディレクトリ
    character(*),parameter :: datadir_output = "/data/sht/nakanog/IHT_drop_d70_we10/contribution/"
    ...
    !無次元数
    real(8),parameter:: We = 10.0d0 !粒子ウエーバー数
...
use glassman
    ...
    allocate(contribution_each_scale(step_num2,(xmax+1)/2 + 1))
...
DO n = step_start, step_end, step_bin
    ...
    write(filename2,*) n
    write(filename3,*) case_num
    filename=datadir_input//trim(adjustl(filename3))//'_'//trim(adjustl(filename2))//'.bin'
    filenameu1=datadir_input2//trim(adjustl(filename3))//'_'//trim(adjustl(filename2))//'_u1.bin'
    filenameu2=datadir_input2//trim(adjustl(filename3))//'_'//trim(adjustl(filename2))//'_u2.bin'
    filenameu3=datadir_input2//trim(adjustl(filename3))//'_'//trim(adjustl(filename2))//'_u3.bin'
    open(10, file=filename, form="unformatted")
    ...
    do k_analysis = 1, (xmax+1)/2 + 1
        ...
        if(comm_rank == 0) then
            k_step = (n - step_start) / step_bin + 1
            contribution_each_scale(k_step,k_analysis) = interaction_all / (dble(xmax+1)*dble(ymax+1)*dble(zmax+1))
        endif
    enddo
ENDDO
    if(comm_rank == 0) then
        write(filename3,*) case_num
        filename = datadir_output//trim(adjustl(filename3))//'_contribution.bin'
        open(110, file=filename, form="unformatted")
        do k_analysis = 1, (xmax+1) / 2
            do k_step = 1, step_num2
                write(110) contribution_each_scale(k_step, k_analysis)
            enddo
        enddo
        close(110)
    endif
