...
module globals
    ...
    !出力ディレクトリ
    character(*),parameter :: datadir = "/data/sht/nakanog/debug3/"
    character(*),parameter :: datadir2 = "/data/sht/nakanog/debug3/fg/"

    !無次元数
    real(8),parameter:: Re = 20000.0d0 !粒子レイノルズ数

    !LESパラメータ
    real(8),parameter:: delta = 4.0d0*ds
...
!================================出力==================================
    ...
    if(comm_rank == 0) then
        if(mod(n,100)==0) then
            if(n == 1) then
                open(10,file="./time_omp_2.d")
                write(10,*) n, time2-time1, u1_procs(1,1,1), g_procs(1,1,1,1)
                close(10)
            else
                open(10,file="./time_omp_2.d",action="write",position="append")
                write(10,*) n, time2-time1, u1_procs(1,1,1), g_procs(1,1,1,1)
                close(10)
            endif
        endif
