...
!===================================初期条件の設定================================================
        ...
        call equilibrium_cal_ini(gphi_procs,phi_procs,p0_procs,lap_phi_procs,grad_phi_procs,grad_u_procs,p_procs,u1_procs,u2_procs,u3_procs,f_procs,g_procs)
    end subroutine ini
    ...
    subroutine equilibrium_cal_ini(gphi_procs,phi_procs,p0_procs,lap_phi_procs,grad_phi_procs,grad_u_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,geq_procs)
        ...
        !$omp end do
        !$omp end parallel
    end subroutine equilibrium_cal_ini

    subroutine equilibrium_cal(gphi_procs,phi_procs,p0_procs,lap_phi_procs,grad_phi_procs,grad_u_procs,p_procs,u1_procs,u2_procs,u3_procs,feq_procs,geq_procs,u1_procs_re,u2_procs_re,u3_procs_re)
        real(8),intent(in):: gphi_procs(1:3,1:3,1:x_procs,1:y_procs,1:zmax+1)
        real(8),intent(in):: phi_procs(0:x_procs+1,0:y_procs+1,0:zmax+2)
        real(8),intent(in):: p0_procs(1:x_procs,1:y_procs,1:zmax+1)
        real(8),intent(in):: lap_phi_procs(1:x_procs,1:y_procs,1:zmax+1)
        ...
        real(8),intent(out):: feq_procs(1:15,0:x_procs+1,0:y_procs+1,0:zmax+2)
        real(8),intent(out):: geq_procs(1:15,0:x_procs+1,0:y_procs+1,0:zmax+2)
        real(8),intent(in):: u1_procs_re(0:x_procs-1, 0:y_procs-1, 0:zmax)
        real(8),intent(in):: u2_procs_re(0:x_procs-1, 0:y_procs-1, 0:zmax)
        real(8),intent(in):: u3_procs_re(0:x_procs-1, 0:y_procs-1, 0:zmax)
        integer i, alpha, beta, xi, yi, zi
                ...
                do xi=1,x_procs
                    do i=1,15
                        ...
                        feq_procs(i,xi,yi,zi) = H(i)*phi_procs(xi,yi,zi) &
                                        ...
                                        - kappaf/6.0d0*(grad_phi_procs(1,xi,yi,zi)**2 &
                                        + grad_phi_procs(2,xi,yi,zi)**2+grad_phi_procs(3,xi,yi,zi)**2)) &
                                        + E(i)*phi_procs(xi,yi,zi)*3.0d0*(cr(1,i)*u1_procs_re(xi-1,yi-1,zi-1) &
                                        + cr(2,i)*u2_procs_re(xi-1,yi-1,zi-1)+cr(3,i)*u3_procs_re(xi-1,yi-1,zi-1)) &
                                        + E(i)*kappaf*gtemp
    ...
    subroutine low_pass(u1_procs,u2_procs,u3_procs,u1_procs_re,u2_procs_re,u3_procs_re,u1_hat,u2_hat,u3_hat,n)
        ...
        !     do yi=1,y_procs
        !         do xi=1,x_procs
        !             err = err + abs(u1_procs_re(xi-1,yi-1,zi-1) - u1_procs(xi,yi,zi))
        !             err = err + abs(u2_procs_re(xi-1,yi-1,zi-1) - u2_procs(xi,yi,zi))
        !             err = err + abs(u3_procs_re(xi-1,yi-1,zi-1) - u3_procs(xi,yi,zi))
        !         enddo
        !     enddo
        ! enddo
        ...
        !         close(103)
        !     else
        !         open(103,file="./err.d",action="write",position="append")
        !         write(103,*) n, err
        !         close(103)
        !     endif
        ! endif
