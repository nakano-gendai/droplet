...
module globals
    ...
    integer,parameter:: xmax = 31 !ｘ方向格子数（０から数える）
    integer,parameter:: ymax = 31 !ｙ方向格子数（０から数える）
    integer,parameter:: zmax = 31 !ｚ方向格子数（０から数える）
    !並行して計算する数
    ...
    character(*),parameter :: datadir = "/data/sht/nakanog/LES12/"
    character(*),parameter :: datadir2 = "/data/sht/nakanog/LES12/fg/"

    !LESパラメータ
    real(8),parameter:: delta = 1.0d0*ds
    real(8),parameter:: Cs = 0.17d0

    !無次元数
    real(8),parameter:: We = 3.0d0 !粒子ウエーバー数
    real(8),parameter:: Re = 80000.0d0 !粒子レイノルズ数
    ...
    real(8),parameter:: D_vortex = 15.5d0 !渦の大きさ
...
contains
    ...
    !========================並列数・コミュニケータ分割・通信先設定========================
        ...
        Nx = 4 !x方向の並列数（ただし，Nx/=comm_procs）
...
!================================乱数の発生（標準正規分布）=====================================
    ...
    subroutine ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs,forcex,forcey,forcez,strain_procs,grad_u_procs,nue,tensor,g_procs,geq_procs)
        ...
        allocate(tensor(1:3,1:3,1:x_procs,1:ymax+1,1:z_procs))
        ...
        call tensor_cal(tensor,g_procs,geq_procs)
        Q = 0.0d0
        S = 0.0d0
        do zi = 1, z_procs
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    ...
                    do beta = 1, 3
                        do alpha = 1, 3
                            Q = Q + tensor(alpha,beta,xi,yi,zi) * tensor(alpha,beta,xi,yi,zi)
                        enddo
                    enddo
                    nu_procs(xi,yi,zi) = (phi_procs(xi,yi,zi)-phi1)/(phi2-phi1)*(nu2-nu1) + nu1
                    S = (nu_procs(xi,yi,zi)**2.0d0 + 18.0d0*Cs*Cs*delta*delta*Q**0.5d0)**0.5d0 - nu_procs(xi,yi,zi)
                    S = S / (6.0d0*Cs*Cs*delta*delta)
                    nue(xi,yi,zi) = Cs * Cs * delta * delta * S
    ...
    subroutine tensor_cal(tensor,g_procs,geq_procs)
        real(8),intent(inout) :: tensor(1:3,1:3,1:x_procs,1:ymax+1,1:z_procs)
        real(8),intent(in) :: g_procs(1:15,0:x_procs+1,0:ymax+2,0:z_procs+1)
        real(8),intent(in) :: geq_procs(1:15,0:x_procs+1,0:ymax+2,0:z_procs+1)
        integer xi, yi, zi, i, alpha, beta
        ...
        do zi = 1, z_procs
            do yi = 1, ymax+1
                do xi = 1, x_procs
                    do beta = 1, 3
                        do alpha = 1, 3
                            tensor(alpha,beta,xi,yi,zi) = 0.0d0
                            do i = 1, 15
                                tensor(alpha,beta,xi,yi,zi) =  tensor(alpha,beta,xi,yi,zi) &
                                                            + ( cr(alpha,i)*cr(beta,i) * (g_procs(i,xi,yi,zi) - geq_procs(i,xi,yi,zi)) )
                            enddo
                        enddo
                    enddo
                enddo
    ...
    subroutine physics(phi_procs,p_procs,u1_procs,u2_procs,u3_procs,f_procs,g_procs,nu_procs,taug_procs,fnext_procs,gnext_procs,nue,tensor,geq_procs)
        real(8),intent(inout):: phi_procs(0:x_procs+1,0:ymax+2,0:z_procs+1)
...
!================================初期条件・配列のallocate======================================
    ...
    call ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs,forcex,forcey,forcez,strain_procs,grad_u_procs,nue,tensor,g_procs,geq_procs)
...
!==========================物理量の計算(OMP)=====================================================
    call physics(phi_procs,p_procs,u1_procs,u2_procs,u3_procs,f_procs,g_procs,nu_procs,taug_procs,fnext_procs,gnext_procs,nue,tensor,geq_procs)
!================================出力==================================
