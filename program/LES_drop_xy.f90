...
module globals
    ...
    integer,parameter:: step_input_file_num = 2400000 !入力する乱流場のstep
    !入力ディレクトリ
    character(*),parameter :: datadir_input = "/data/sht/nakanog/DNS_turbulence_256_re80000/fg/"
    !出力ディレクトリ
    character(*),parameter :: datadir_output = "/data/sht/nakanog/debug_xy/"
    character(*),parameter :: datadir_output_fg = "/data/sht/nakanog/debug_xy/fg/"
    integer,parameter:: step_output = 4000
    integer,parameter:: step_putput_fg = 1000
    ...
    real(8),parameter:: D = 15.0d0 !設置する液滴直径
    ...
    real(8),parameter:: sigma = 3.8d-6 !界面張力
...
!===================================初期条件の設定================================================
    ...
    subroutine ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs,forcex,forcey,forcez,strain_procs,grad_u_procs,nue)
    ...
    subroutine MPI_boundary_g(feq_procs,geq_procs)
    ...
    subroutine low_pass(u1_procs,u2_procs,u3_procs,u1_procs_re,u2_procs_re,u3_procs_re,u1_hat,u2_hat,u3_hat)
        ...
        real(8) k_abs
        ! real(8),allocatable :: enesupe(:), enesupe_sum(:)
        ! integer i
        ...
        ! kmax_abs = ( (dkx*dble(xmax+1)/2.0d0)**2 + (dky*dble(ymax+1)/2.0d0)**2 + (dkz*dble(zmax+1)/2.0d0)**2 )**0.5d0
        ! allocate(enesupe( floor(kmax_abs / dk) + 1))
        ! allocate(enesupe_sum( floor(kmax_abs / dk) + 1))
        ...
        !             if(k3 == 0) then
        !                 enesupe(k_index) = enesupe(k_index) + 0.5d0 * (abs(u1_hat(k1,k2,k3))**2 + abs(u2_hat(k1,k2,k3))**2 + abs(u3_hat(k1,k2,k3))**2) / dkx
        !             else 
        !                 enesupe(k_index) = enesupe(k_index) + 0.5d0 * (abs(u1_hat(k1,k2,k3))**2 + abs(u2_hat(k1,k2,k3))**2 + abs(u3_hat(k1,k2,k3))**2) * 2.0d0 / dkx
        !             endif
        !         enddo
        !     enddo
        ! enddo

        ! call MPI_Reduce(enesupe(1), enesupe_sum(1), floor(kmax_abs / dk) + 1, MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
        ! if(comm_rank==0) then
        !     open(37,file ="./enesupe.d")
        !     do i=1,size(enesupe_sum)
        !         write(37,*) dble(i)-1.0d0, enesupe_sum(i)
        !     enddo
        !     close(37)
        ! endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!設定!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    call ini_op(taug_procs,nu_procs,phi_procs,kinetic_procs,forcex,forcey,forcez,strain_procs,grad_u_procs,nue)
